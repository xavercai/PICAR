 ...
 */
 /*
【20260724 NewHW0110723_v3perfectFiledebugEngineDBwebEVO版本变更记录,修改完成后，save本版本，然后生成一个新的名字：20260724TEST】
变更内容汇总：
1、电机力矩均衡优化
...
4、个体生命周期结束行为新增
   - 个体评测正常结束 / 混沌超时终止后，原地后退2秒，停机后载入下一个个体
   - 使用新增状态STATE_BACKWARD_RESET非阻塞实现，不阻塞Web通讯、传感器采集
   - 后退速度、持续时长使用常量统一配置，方便现场微调
    ...
    void update() {
        ...
        case STATE_BACKWARD_RESET:
        {
            ...
            if (now - backwardResetStart >= BACKWARD_RESET_DURATION)
            {
                ...
                state = STATE_SCORE; // 后退结束，进入打分，切换下一个体
            }
            break;
        }
        case STATE_SCORE:
...
private:
    enum State {
        ...
        const uint32_t BACKWARD_RESET_DURATION = 2000;
    ...
    void runTest() {
        ...
        if (now - testStart >= TEST_DURATION_MS) {
            ...
            Logger::log("✅ Test complete, start backward reset 1s");
            return;
        }
