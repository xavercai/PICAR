 ...
 * 版本：    v6.0-ESP32S3-GP2Y0A21YK0F-CALIBRATED
 * 适配日期: 2026-08-06
 * 
 * ================================================================
 * 【v6.0 更新说明】
 * ================================================================
 * 1. 传感器引脚修正：左=GPIO4，右=GPIO5（与硬件丝印统一）
 * 2. 阈值基于实测校准：10cm→3200，20cm→1550
 * 3. 新增三级障碍分级：安全(<600) / 注意(600~1400) / 危险(>1400)
 * 4. 优化控制逻辑：速度更合理，转向响应更灵敏
 * 5. 降低混沌触发门槛：卡死判断从3秒降至2秒
 * 6. 增加距离分级速度控制：越近越慢，越远越快
 * 7. 修正所有注释与硬件实际对应
 * 修正v5.9 （文件名20260802Sharp21SensorV0.ino)里面无避障记录，无死局和混沌的问题
 * 
 * 【实测校准数据】
 * 距离    | ADC Raw 值
 * 10cm    | 3200
 ...
 * 
 * 【阈值设定】
 * DANGER_ZONE_THRESH  = 2600  (约12cm，紧急)
 * OBSTACLE_ADC_THRESH = 1400  (约22cm，避障触发)
 * CLEAR_ADC_THRESH    = 600   (约70cm，安全)
 * ================================================================
 */
...
// ===================== 硬件引脚宏【丝印 GPIO 一一对应】 =====================
// GP2Y0A21YK0F 模拟测距 ADC 输入
#define PIN_SENSOR_LEFT    4     // 左传感器 OUT → IO4 (ADC1_CH3)
#define PIN_SENSOR_RIGHT   5     // 右传感器 OUT → IO5 (ADC1_CH4)

// 悬空物理热噪声采样引脚（论文规定物理随机变异源）
#define PIN_NOISE_SOURCE   6     // IO6 悬空，用于生成真随机数

// 障碍物电平外部中断预留引脚（当前未使用）
#define PIN_OBSTACLE_INT   39    // IO39 预留

// 板载 LED 指示灯
#define PIN_LED            14    // IO14 板载指示灯

// 左电机（AT8236）
#define PIN_LEFT_PWM       9     // IO9  IN1 = PWM
#define PIN_LEFT_DIR2      46    // IO46 IN2 = 方向

// 右电机（AT8236）
#define PIN_RIGHT_PWM      16    // IO16 IN1 = PWM
#define PIN_RIGHT_DIR2     15    // IO15 IN2 = 方向

// 左编码器 AB 相
#define PIN_LEFT_ENC_A     18    // IO18
#define PIN_LEFT_ENC_B     17    // IO17

// 右编码器 AB 相
#define PIN_RIGHT_ENC_A    11    // IO11
#define PIN_RIGHT_ENC_B    10    // IO10
...
#define STUCK_THRESHOLD_MS  2000   // 降低到2秒，更容易触发混沌
...
// ===================== GP2Y0A21YK0F ADC原始阈值（基于实测校准） =====================
// 实测数据：10cm→3200, 20cm→1550, 无障碍→100~200
#define CLEAR_ADC_THRESH       600    // ADC < 600 判定无障碍 (>70cm)
#define OBSTACLE_ADC_THRESH    1400   // ADC > 1400 判定有障碍 (<22cm)
#define DANGER_ADC_THRESH      2600   // ADC > 2600 判定紧急危险 (<12cm)
...
public:
    static void init() {
        ...
        Logger::log("   引脚：左=IO4，右=IO5");
        Logger::log("   原则：去标定、去滤波、去PID");
        Logger::logf("   阈值：危险=%d 障碍=%d 安全=%d", 
                     DANGER_ADC_THRESH, OBSTACLE_ADC_THRESH, CLEAR_ADC_THRESH);
        Logger::log("   实测校准：10cm→3200, 20cm→1550");
    }
    ...
    static int getObstacleLevel() {
        ...
        if (maxVal > DANGER_ADC_THRESH) return 3;     // 紧急危险 (<12cm)
        if (maxVal > OBSTACLE_ADC_THRESH) return 2;    // 有障碍 (12~22cm)
        if (maxVal > CLEAR_ADC_THRESH) return 1;       // 注意 (22~70cm)
        return 0;                                       // 安全 (>70cm)
    }
    ...
    static void printStatus() {
        Logger::logf("  L:%d  R:%d  Level:%d", left(), right(), getObstacleLevel());
    }
};
...
private:
    ...
    void runTest() {
        ...
        if (!inChaos) {
            // ============================================================
            // 【核心控制逻辑】基于实测校准的GP2Y0A21YK0F
            // 距离越近 ADC 值越大：10cm→3200, 20cm→1550, 安全→<600
            // ============================================================
    ...
    void runChaos() {
        ...
        if (chaosStep < CHAOS_SAMPLE_COUNT) {
            ...
            if (now - lastChaosTime >= CHAOS_STEP_MS) {
                ...
                // 脱困判断：传感器变安全 或 编码器有显著移动
                if (Sensors::isClear() || Encoders::totalDelta() > 20) {
                    chaosEscaped = true;
...
<head>
    ...
    <title>Car Evolution v6.0 - GP2Y0A21YK0F 校准版</title>
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .calib-box { background:#0a0a1a; padding:8px 12px; border-radius:6px; margin:8px 0; font-size:12px; color:#888; }
        .calib-box span { color:#00d4ff; }
    </style>
</head>
<body>
<div class='box'>
    <h1>🚗 Evolution v6.0</h1>
    <div class='subtitle'>
        <span class='badge-physical'>⚡ 去滤波·去标定·去PID</span>
        <span class='badge-control'>🔬 只观察·不优化</span>
        <span style='color:#888;font-size:12px;'>| GP2Y0A21YK0F 校准版</span>
    </div>
    
    <div class='calib-box'>
        📐 阈值：安全 &lt;600 | 障碍 &gt;1400 | 危险 &gt;2600
        <br>📊 实测：10cm→3200 | 20cm→1550
    </div>
    ...
    <div class='sensor-box'>
        ...
        <div class='sensor-row'>
            <span class='sensor-label'>🚧 避障计数</span>
...
<div class='footer'>
    <span>✅ v6.0 GP2Y0A21YK0F 校准版</span> | 纯模拟方案 | 去滤波·去标定·去PID
</div>
    ...
    fetch('/status')
        ...
        .then(t => {
            ...
            if(data['sensor_L'] !== undefined) {
                ...
                el.textContent = val;
                if(val > 2600) el.className = 'sensor-low';
                else if(val > 1400) el.className = 'sensor-high';
                else el.className = 'sensor-high';
            }
...
void setup() {
    Logger::init();
    Logger::log("========================================");
    Logger::log("  Car Evolution v6.0 - GP2Y0A21YK0F");
    Logger::log("  纯模拟方案 | 严格遵循三原则");
    Logger::log("  设计哲学：去滤波·去标定·去PID");
    Logger::log("========================================");
    Logger::log("  ✅ 传感器：GP2Y0A21YK0F 模拟红外测距");
    Logger::log("  ✅ 引脚：左=IO4，右=IO5");
    Logger::log("  ✅ 输出：原始ADC值 (0-4095)");
    Logger::log("  ✅ 物理噪声：悬空ADC引脚 (IO6)");
    Logger::log("  ✅ 实测校准：10cm→3200，20cm→1550");
    Logger::log("  ✅ 阈值：安全<600，障碍>1400，危险>2600");
 ...
 * 项目名称：仿草履虫应激机制 - 小车避障进化验证系统
 * 硬件平台：ESP32-S3-WROOM-1U Maturo一体底板（内置AT8236电机驱动）
 * 版本：v5.6-ESP32S3-RAM-ONLY-GP2Y0A21YK0F
 * 适配日期: 2026-08-02
 ...
 * GPIO35    PCB IO35    左GP2Y0A21YK0F模拟输出OUT（ADC）
 * GPIO36    PCB IO36    右GP2Y0A21YK0F模拟输出OUT（ADC）
 * GPIO6     PCB IO6     悬空热噪声专用引脚（物理真随机变异源）
 * GPIO39    PCB IO39    预留障碍物电平外部中断（备用）
 * GPIO14    PCB IO14    板载LED指示灯
 * GPIO9     PCB IO9     左电机PWM调速
 * GPIO46    PCB IO46    左电机转向DIR2
 * GPIO16    PCB IO16    右电机PWM调速
 * GPIO15    PCB IO15    右电机转向DIR2
 ...
 * 左传感器：VCC→HW131 5V  GND→公共GND  OUT→GPIO35
 * 右传感器：VCC→HW131 5V  GND→公共GND  OUT→GPIO36
 * ⚠️接线红线：
