 ...
 * 文件：OEETestFinal_v9.7.1.ino
 * 版本：v9.7.1-ChaosTimeoutFix
 * 最后更新：2026-08-16
 * 
 * 【版本演进日志】- 按时间顺序完整记录
 * ================================================================
 * 
 * v7.3-fixed-v2 - 经典遗传算法基线 (2026-08-09 ~ 2026-08-11)
 * ────────────────────────────────────────────────────────────────
 *   设计哲学：经典遗传算法(GA)，固定适应度函数，精英保留，收敛到最优。
 *   基因型：5个固定参数 (k_turn, speed_bias, chaos_threshold,
 *           recovery_mode, preferred_action)
 * 
 *   编译错误修复 (v7.3-fixed → v7.3-fixed-v2)：
 *   1. EvolutionEngine 重构为单例+静态包装模式
 ...
 * 
 * v8.0 - 开放式进化重构 "物理结构即算法" (2026-08-11)
 * ────────────────────────────────────────────────────────────────
 *   设计哲学转变：从"目标导向优化"转向"行为多样性探索"。
 ...
 * 
 * v8.5-final - 硬件适配与编译修复 (2026-08-12)
 * ────────────────────────────────────────────────────────────────
 ...
 *   5. 右电机正反转方向与 v5.9 对齐
 *   SPIFFS状态：此时SPIFFS被禁用，数据仅保存在RAM中。
 * 
 * v8.6 - 非阻塞状态机重构 (2026-08-13 ~ 2026-08-14)
 * ────────────────────────────────────────────────────────────────
 ...
 *     - Web控制台在动作执行期间保持响应
 * 
 * v8.6.4 - 实机运行数据分析与关键Bug修复 (2026-08-14)
 * ────────────────────────────────────────────────────────────────
 *   实机运行数据：
 ...
 * 
 *   存储空间需求：总计约1MB (适合1.5-2MB SPIFFS)
 * 
 * v8.8 - 卡死感知 + 向死而生混沌机制 (2026-08-15)
 * ────────────────────────────────────────────────────────────────
 *   问题背景：v8.0-v8.7 重构时，原有的混沌/卡死检测逻辑被"行为表规则替代"
 *   但没有等价功能补上。进化没有选择压力，退化为随机游走。
 * 
 *   新增功能：
 *     1. 卡死检测（硬编码基础设施）
 ...
 *     - 脱困行为 = 基因中的规则控制（可进化）
 * 
 * v8.9-robust-stable - 鲁棒性增强 (2026-08-15)
 * ────────────────────────────────────────────────────────────────
 ...
 *     修复：所有状态都记录帧日志
 * 
 * v8.9-chaos - 混沌机制完整实现 (2026-08-15)
 * ────────────────────────────────────────────────────────────────
 *   问题背景：v8.8 中加入的 COND_STUCK 和 COND_ESCAPE 条件类型，
 ...
 *     6. Web界面增加卡死/脱困状态显示
 * 
 * v8.9-chaos-v2 ~ v8.9-chaos-v6 - 向死而生混沌机制演进 (2026-08-15)
 * ────────────────────────────────────────────────────────────────
 *   核心设计哲学：
 ...
 *     模式1：编码器差值卡死 - diff > 15 且持续增大，两轮都在转，持续1秒
 *     模式2：单轮卡死 - 一个轮子转(>20)，另一个轮子停(<3)，持续1秒
 *     模式3：大差值兜底 - diff > 30，任意轮子在转，持续1秒
 * 
 ...
 *     → 物理运动恢复 → 代码重新接管
 * 
 * v8.9-chaos-v6 - 自身对照实验设计 (2026-08-16)
 * ────────────────────────────────────────────────────────────────
 *   论文背景：论文实验设计章节提出了"自身对照"方法论——
 *   以混沌触发为时间分界线，将同一硬件上的行为数据划分为
 *   基线组（Chaos-Free）和实验组（Chaos-Intervention），
 *   通过差分分析消除硬件偏差与环境噪声干扰。
 * 
 *   本次修改 (14处)：
 *     1. FrameLogEntry 结构体 - 将 reserved:6 拆分为 chaosActive:1 + reserved:4
 *     2. 新增 ChaoticTestRecord 结构体 (56字节/条)
 *     3. MotorController 类 (5处) - init(), startChaos(), endChaos(), 
 *        logFrame(), 14个公开访问器 + resetChaosStatistics()
 *     4. EvolutionEngine 类 (2处) - startCurrentTestImpl() 调用 resetChaosStatistics()
 *        endCurrentTestImpl() 收集统计数据并保存
 *     5. GeneStorage + RobustStorage + Web (5处) - saveChaosRecord(),
 *        addChaosRecord(), getChaosHistoryCSV(), /download/chaos 端点
 * 
 *  自身对照数据流：
 ...
 * 
 * v9.3-OEETestV3 - 当前状态 (2026-08-16)
 * ────────────────────────────────────────────────────────────────
 *   完整功能清单：
 ...
 * 
 * v9.4-ChaosFix - 混沌触发修复 (2026-08-16)
 * ────────────────────────────────────────────────────────────────
 *   问题现象：即使编码器差值超过100甚至200，混沌机制从未被触发
 * 
 *   问题根因：MotorController::update() 中，卡死检测和混沌触发逻辑
 *   被放在 if (!ruleActive) 代码块内部。
 *   因为 ruleActive 一直为 true，永远跳不到触发混沌的代码。
 * 
 *   修复方案：
 *     1. 将卡死检测逻辑从 if (!ruleActive) 块中移出 → 每帧都执行
 *     2. 将混沌触发判断移到 if (!ruleActive) 块外部
 *     3. 混沌触发后立即 return，避免与规则执行冲突
 *     4. 增加详细调试日志
 * 
 * v9.5-DataAuditFix - 数据审计修复 (2026-08-16)
 * ────────────────────────────────────────────────────────────────
 *   修复1：增加帧日志容量 - FRAME_LOG_SIZE 256 → 4096 (可存储40秒数据)
 *   修复2：删除重复写入 - endCurrentTestImpl() 中删除 GeneStorage::saveHistoryRecord()
 *   修复3：新增独立下载端点
 ...
 *   修复5：单个体完整数据包
 *     /download/individual?gen=N&id=M 返回完整数据包
 * 
 * v9.7-MotorBalanceFix - 电机平衡修复 (2026-08-16)
 * ────────────────────────────────────────────────────────────────
 ...
 * 
 * v9.7.1-ChaosTimeoutFix - 混沌超时修复 (2026-08-16) ★ 当前版本
 * ────────────────────────────────────────────────────────────────
 *   [核心修复]
 *   ● 混沌模式基于编码器实际运动检测恢复（替代虚假信号检测）
 *   ● 混沌超时保护：5秒无运动强制退出 + 脱困冲撞
 *   ● 电机PWM不平衡补偿 (LEFT_FWD=1.00, RIGHT_FWD=1.25)
 *   ● 完整串口命令和Web下载界面
 *   ● 新增 popall <代> 命令：一键查看一代所有个体汇总数据
 * 
 *   [技术细节]
 *   - CHAOS_FORCE_TIMEOUT_MS = 5000ms 强制退出
 *   - CHAOS_ESCAPE_PWM = 200 脱困冲撞
 *   - CHAOS_RECOVER_STABLE_FRAMES = 20 稳定帧检测
 *   - 右轮前进增益从1.25调整为1.30
 * 
 * 【系统架构】
 * ================================================================
 ...
 * ================================================================
 *   混沌控制：
 *     CHAOS_TIMEOUT_MS        = 2000ms  卡死检测超时
 *     CHAOS_FORCE_TIMEOUT_MS  = 5000ms  混沌强制退出
 *     CHAOS_ESCAPE_PWM        = 200     脱困冲撞PWM
 *     CHAOS_NOISE_AMPLIFIER   = 180     噪声放大系数
 * 
 *   电机补偿 (v9.7.1)：
 *     LEFT_FWD_GAIN   = 1.00    左轮前进增益
 *     RIGHT_FWD_GAIN  = 1.30    右轮前进增益（补偿）
 *     LEFT_REV_GAIN   = 1.30    左轮反转增益
 *     RIGHT_REV_GAIN  = 1.30    右轮反转增益
 * 
 *   进化参数：
 *     POPULATION_SIZE = 16      种群大小
 *     TEST_DURATION_MS = 30000  每代测试时长
 *     NOVELTY_K_NEAREST = 5     K近邻数
 *     NOVELTY_ADD_THRESHOLD = 0.15  档案添加阈值
 * 
 * 【硬件引脚映射】
 * ================================================================
 *   PIN_SENSOR_LEFT    = 4     左红外传感器
 *   PIN_SENSOR_RIGHT   = 5     右红外传感器
 *   PIN_NOISE_SOURCE   = 35    噪声源ADC (GPIO35)
 *   PIN_LEFT_PWM       = 9     左电机PWM
 *   PIN_LEFT_DIR2      = 46    左电机方向
 *   PIN_RIGHT_PWM      = 16    右电机PWM
 *   PIN_RIGHT_DIR2     = 15    右电机方向
 *   PIN_LEFT_ENC_A     = 18    左编码器A
 *   PIN_LEFT_ENC_B     = 17    左编码器B
 *   PIN_RIGHT_ENC_A    = 11    右编码器A
 *   PIN_RIGHT_ENC_B    = 10    右编码器B
 *   PIN_LED            = 14    状态LED
 *   PIN_OBSTACLE_INT   = 39    中断输入
 * 
 * 【串口命令】
 * ================================================================
 *   ls / list     - 列出SPIFFS所有文件
 *   status        - 显示系统状态
 *   frame <代> <个体>  - 解析帧日志
 *   pop <代>      - 解析种群快照
 *   chaos <代> <个体>  - 解析混沌记录
 *   popall <代>   - ★ 显示一代所有个体汇总数据
 *   help          - 显示帮助
 * 
 * 【Web界面】
 * ================================================================
 ...
 *   功能：实时监控、电机控制、实验控制、数据导出
 * 
 * 【编译信息】
 * ================================================================
 *   开发板: ESP32 Dev Module
 *   Flash Size: 4MB
 *   Partition: SPIFFS (1.2MB APP / 1.5MB SPIFFS)
 * 
 * 【数据流总览】
 * ================================================================
 * 
...
#define RIGHT_FWD_GAIN      1.30f   // 右轮前进加力
#define LEFT_REV_GAIN       1.30f   // 左轮反转增益（左转力度不够调大）
#define RIGHT_REV_GAIN      1.30f   // 右轮反转增益（右转力度不够调大）
#define LEFT_DEADZONE       35
#define RIGHT_DEADZONE      20      // 右轮死区降低
...
// 固件版本
// ================================================================
#define FIRMWARE_VERSION "v9.7.1-ChaosTimeoutFix"
#define VERSION_MARKER_FILE "/version_" FIRMWARE_VERSION ".mrk"

// ================================================================
// 卡死检测参数
// ================================================================
#define ENCODER_DIFF_THRESHOLD   30
#define ENCODER_STUCK_DURATION   1000
#define ENCODER_DIFF_MIN         5
#define WHEEL_SPIN_THRESHOLD     20
...
#define CHAOS_FORCE_TIMEOUT_MS  5000   // 5秒无运动强制退出
#define CHAOS_ESCAPE_PWM        200    // 脱困冲撞PWM
...
public:
    ...
    static void init() {
        ...
        ledcWrite(LEDC_CHANNEL_RIGHT, 0);
        stopMotors();
        Logger::log("Motor Controller initialized (v9.7.1)");
    ...
    static void updateChaos() {
        ...
        if (hasPhysicalMovement) {
            ...
            if (stableFrames >= CHAOS_RECOVER_STABLE_FRAMES) {
                Logger::logf("✅ Chaos recovery: encoder movement detected! totalDelta=%ld", totalDelta);
                endChaos();
                stableFrames = 0;
                return;
            }
        ...
        // --- ★★★ 超时保护：5秒无运动强制退出 + 脱困冲撞 ★★★ ---
        if (millis() - chaosStartTime > CHAOS_FORCE_TIMEOUT_MS) {
            Logger::log("⚠️ Chaos timeout! No movement for 5s, force exit with escape maneuver.");
            // 强力脱困尝试：前后冲撞
...
<h1>🚗 进化小车 <span style="font-size:14px;color:#8b949e;">v9.7.1-ChaosTimeoutFix</span></h1>
<div class="notice">✅ 混沌超时保护 | 5秒无运动强制退出 | 串口输入 popall 1 查看一代数据</div>
<div class="grid">
...
'<br>混沌时长: <span class=val>'+d.chaosDuration+'ms</span>'+
...
void setup() {
    ...
    Logger::log("  Chaos Timeout Fix - v9.7.1");
    Logger::log("  5s force exit if no encoder movement");
