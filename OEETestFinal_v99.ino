 ...
 * 文件：OEETestFinal_v9.8.ino
 * 版本：v9.8-StateMachineRefactor
 * 最后更新：2026-08-16
 * 
 * 【v9.8 状态管理重构】- 2026-08-16
 * ────────────────────────────────────────────────────────────────
 * 
 *   改进1：单一状态枚举替代多个布尔标志
 *   ────────────────────────────────────────
 *   问题：原系统使用 stuckFlag、chaosActive、escapeActive、ruleActive 
 *         四个独立布尔标志，状态组合存在歧义（如 stuckFlag=true 且 
 *         chaosActive=true 同时存在的不合理状态）。
 * 
 *   解决：引入 MotorState 枚举，包含 5 个互斥状态：
 *         STATE_IDLE(0)     - 空闲
 *         STATE_WALKING(1)  - 基因规则驱动行走
 *         STATE_STUCK(2)    - 卡死检测中
 *         STATE_CHAOS(3)    - 混沌模式（物理噪声驱动）
 *         STATE_ESCAPING(4) - 脱困规则执行中
 *         
 *         用单一 static MotorState motorState 变量替代全部布尔标志。
 * 
 *   收益：状态互斥，无歧义；状态转换清晰可追踪；便于调试和扩展。
 * 
 * 
 *   改进2：统一封装卡死状态清除逻辑
 *   ────────────────────────────────────────
 *   问题：stuckFlag = false 的清除逻辑分散在 stopMotors()、resetStuck()、
 *         update() 等 5 处，修改时容易遗漏，导致状态不一致。
 * 
 *   解决：新增 clearStuckState() 函数，集中处理所有卡死状态清除：
 *         - 判断当前状态是否为 STUCK 或 CHAOS
 *         - 自动切换到 IDLE
 *         - 重置 stuckStartTime
 *         - 清除 escapeActiveCompat
 * 
 *   收益：单一修改点，确保所有清除逻辑一致，避免状态残留。
 * 
 * 
 *   改进3：帧日志增加 state 字段
 *   ────────────────────────────────────────
 *   问题：帧日志仅记录 chaosActive 标志 (1bit)，无法还原完整状态机
 *         阶段，离线分析时无法判断卡死/脱困/混沌的精确时序。
 * 
 *   解决：FrameLogEntry 结构体调整：
 *         - reserved:5 拆分为 state:3 + reserved:2
 *         - 每帧记录当前 motorState 值 (0-4)
 *         - 帧大小从 11 字节变为 11 字节（不变，利用原有保留位）
 * 
 *   收益：离线可精确重建状态机时间线，支持论文数据分析。
 * 
 * 
 *   【状态转换图】v9.8
 *   ──────────────────────────────────────────────────────
 ...
 *   【字段变更】
 *   ──────────────────────────────────────────────────────
 *   FrameLogEntry (11 bytes)
 *   ┌────────────┬────────────┬────────────┬──────────┐
 *   │ 字段       │ 原大小     │ 新大小     │ 说明     │
 *   ├────────────┼────────────┼────────────┼──────────┤
 *   │ timestamp  │ 4 bytes    │ 4 bytes    │ 不变     │
 *   │ sensorL    │ 2 bytes    │ 2 bytes    │ 不变     │
 *   │ sensorR    │ 2 bytes    │ 2 bytes    │ 不变     │
 *   │ motorL PWM │ 1 byte     │ 1 byte     │ 不变     │
 *   │ motorR PWM │ 1 byte     │ 1 byte     │ 不变     │
 *   │ directionL │ 1 bit      │ 1 bit      │ 不变     │
 *   │ directionR │ 1 bit      │ 1 bit      │ 不变     │
 *   │ chaosActive│ 1 bit      │ 1 bit      │ 保留兼容 │
 *   │ state      │ 无         │ 3 bits     │ ★ 新增   │
 *   │ reserved   │ 5 bits     │ 2 bits     │ 缩小     │
 *   └────────────┴────────────┴────────────┴──────────┘
 * 
 * ================================================================
 *//*
 ...
 * ================================================================
 * 项目名称：仿草履虫应激机制 - 脱困能力进化系统
 * 文件：OEETestFinal_v9.8_RAMLogBuffer.ino
 * 版本：v9.8-RAMLogBuffer
 * 最后更新：2026-08-16
 * 
 * 【版本演进日志】- 按时间顺序完整记录
 * ================================================================
 ...
 *    死亡时: endCurrentTestImpl() → 收集全部统计 → 写入个体CSV + 汇总CSV
 * 
 * v9.3-OEETestV3 - 完整功能基线 (2026-08-16)
 * ────────────────────────────────────────────────────────────────
 *   完整功能清单：
 *     ✅ Novelty Search 新奇度驱动进化
 *     ✅ 可变长度行为表基因 (2~16条IF-THEN规则)
 ...
 *   5. FRAME_LOG_SIZE=4096 (可存40秒数据)
 * 
 * v9.7.1-ChaosTimeoutFix - 混沌超时修复 (2026-08-16)
 * ────────────────────────────────────────────────────────────────
 *   [核心修复]
 *   ● 混沌模式基于编码器实际运动检测恢复（替代虚假信号检测）
 *   ● 混沌超时保护：5秒无运动强制退出 + 脱困冲撞
 ...
 *   - 右轮前进增益从1.25调整为1.30
 * 
 * v9.8-Refactor - 状态机重构 + 单一状态变量 (2026-08-16)
 * ────────────────────────────────────────────────────────────────
 *   [核心重构]
 *   ● 引入 MotorState 枚举 (IDLE/WALKING/STUCK/CHAOS/ESCAPING)
 *   ● 用单一状态变量替代 4 个布尔标志，消除状态组合歧义
 *   ● 统一封装 clearStuckState() 函数
 *   ● FrameLogEntry 增加 state 字段 (3bit)
 *   ● 状态切换自动记录时间和距离快照
 * 
 *   [架构改进]
 *   - 状态机更加清晰，易于调试和扩展
 ...
 *   - 减少状态不一致导致的bug
 * 
 * v9.8-RAMLogBuffer - RAM动态日志缓冲 + SPIFFS滚动覆盖 (2026-08-16) ★ 当前版本
 * ────────────────────────────────────────────────────────────────
 *   [核心功能]
 *   ● RAMLogBuffer 类：6000帧环形缓冲区，Web/串口动态提取
 *   ● RollingStorage 类：自动滚动覆盖，保留最近5代Top2
 *   ● 增量保存：每500帧自动写入SPIFFS (P1修复)
 *   ● CRC32校验：文件头增加校验字段 (P3修复)
 *   ● 滑动窗口卡死检测：10帧窗口验证 (P4修复)
 *   ● 存储空间预警：getFreeSpace() 实时监控 (P2修复)
 *   ● 帧日志差分压缩：每帧压缩1字节 (P0-①修复)
 * 
 *   [容量优化]
 *   - SPIFFS占用: 12×3000×2×5 = 360KB (原11MB)
 *   - RAM占用: 12×6000 = 72KB
 *   - 压缩后: 11×3000×2×5 = 330KB
 * 
 *   [P0-P4 完整处理状态]
 *   ┌────────┬─────────────────────┬──────────┐
 *   │ 优先级 │ 问题               │ 状态     │
 ...
 * 
 *   [新增Web接口]
 *   - /ramlog?start=N&end=M  文本格式提取RAM日志
 *   - /ramlog/download?start=N&end=M  二进制下载
 *   - /storage/status  存储状态查询
 * 
 *   [新增串口命令]
 *   - ramlog <start> <end>  提取RAM日志
 *   - ramlog all  提取全部
 *   - storage  查看存储状态
 * 
 * 【系统架构】v9.8-RAMLogBuffer
 * ================================================================
 *   ┌─────────────────────────────────────────────────────────────────────┐
 *   │              仿草履虫应激进化系统 v9.8-RAMLogBuffer                │
 *   ├─────────────────────────────────────────────────────────────────────┤
 *   │                                                                     │
 *   │  ┌─────────────────────────────────────────────────────────────────┐│
 *   │  │                    RAM 层 (实时可查)                          ││
 *   │  │  ┌──────────────────────────────────────────────────────────┐ ││
 *   │  │  │  RAMLogBuffer (6000帧环形缓冲区, 72KB)                 │ ││
 *   │  │  │  ├─ Web: /ramlog?start=N&end=M 实时提取                │ ││
 *   │  │  │  ├─ 串口: ramlog 0 100 实时提取                        │ ││
 *   │  │  │  └─ 个体结束前数据始终可查                              │ ││
 *   │  │  └──────────────────────────────────────────────────────────┘ ││
 *   │  └─────────────────────────────────────────────────────────────────┘│
 *   │                              │                                      │
 ...
 * 
 *   RAM日志缓冲 (v9.8)：
 *     RAM_LOG_BUFFER_SIZE = 6000    RAM中保留帧数 (≈60秒)
 *     SPIFFS_MAX_GENERATIONS = 5    SPIFFS保留代数
 *     SPIFFS_MAX_INDIVIDUALS = 2    每代保留个体数
 *     INCREMENTAL_SAVE_INTERVAL = 500  增量保存间隔 (帧)
 * 
 * 【硬件引脚映射】
 * ================================================================
 *   PIN_SENSOR_LEFT    = 4     左红外传感器
 *   PIN_SENSOR_RIGHT   = 5     右红外传感器
 *   PIN_NOISE_SOURCE   = 1     噪声源ADC (GPIO1)
 *   PIN_OBSTACLE_INT   = 39    中断输入
 *   PIN_LED            = 14    状态LED
 *   PIN_LEFT_PWM       = 9     左电机PWM
 *   PIN_LEFT_DIR2      = 46    左电机方向
 ...
 *   chaos <代> <个体>  - 解析混沌记录
 *   popall <代>   - 显示一代所有个体汇总数据
 *   ramlog <开始> <结束>  - ★ 提取RAM日志 (文本格式)
 *   ramlog all    - ★ 提取全部RAM日志
 *   storage       - ★ 查看存储状态
 *   help          - 显示帮助
 * 
 * 【Web界面】v9.8
 ...
 *   新增端点:
 *   /ramlog?start=N&end=M     - 文本格式提取RAM日志
 *   /ramlog/download?start=N&end=M  - 二进制下载RAM日志
 *   /storage/status           - 存储状态查询
 * 
 * 【编译信息】
 * ================================================================
 ...
 * │    │   ├─ 滑动窗口卡死检测 (10帧窗口)                              │
 * │    │   ├─ 状态机管理 (IDLE→STUCK→CHAOS)                           │
 * │    │   └─ 规则匹配 → 执行动作                                      │
 * │    ├─ MotorController::logFrame()                                 │
 * │    │   ├─ 写入 frameLog[] (环形, 4096帧)                          │
 * │    │   ├─ 写入 RAMLogBuffer (环形, 6000帧) ★                      │
 * │    │   └─ 每500帧增量保存到SPIFFS ★                               │
 * │    └─ updateChaos() (混沌模式)                                    │
 * │                                                                     │
 * └─────────────────────────────────────────────────────────────────────┘
 ...
 * ┌─ 持久化数据流 ───────────────────────────────────────────────────────┐
 * │                                                                     │
 * │  个体结束 → endCurrentTestImpl():                                   │
 * │    ├─ 新奇度计算 → archive.addIfNovel()                            │
 * │    ├─ HistoryRecord → /oe_history.csv                              │
 * │    ├─ ChaoticTestRecord → /chaos_g{N}_i{X}.csv                     │
 * │    ├─ 基因记录 → /gen_{N}_id_{X}.csv                               │
 * │    ├─ 帧日志 → RollingStorage::saveFrameLog() ★                    │
 * │    │   ├─ CRC32校验 ★                                              │
 * │    │   ├─ 只保留Top2 ★                                             │
 * │    │   └─ 自动滚动覆盖 (最多5代) ★                                 │
 * │    └─ 种群快照 → /pop_gen_{N}.bin (原子写入)                       │
 ...
 * │                                                                     │
 * │  Web/串口请求:                                                      │
 * │    ├─ /ramlog?start=N&end=M → RAMLogBuffer::extractFrames() ★      │
 * │    ├─ /ramlog/download → RAMLogBuffer::extractBinary() ★           │
 * │    ├─ /storage/status → RollingStorage::getStorageStatus() ★       │
 * │    └─ 串口: ramlog 0 100 → RAMLogBuffer::extractFrames() ★        │
 * │                                                                     │
 * └─────────────────────────────────────────────────────────────────────┘
 * 
 * ================================================================
 ...
 * ================================================================
 * 项目名称：仿草履虫应激机制 - 脱困能力进化系统
 * 文件：OEETestFinal_v9.9.ino
 * 版本：v9.9-ChaosFix+WebRefactor
 * 最后更新：2026-08-17
 * 
 * 【v9.9 混沌修复 + Web重构】- 2026-08-17
 * ────────────────────────────────────────────────────────────────
 * 
 *   修复1：混沌永不触发Bug
 *   ─────────────────────────
 *   问题：小车卡死后，Web显示混沌始终为"空闲"，混沌机制从未被触发。
 * 
 *   根因分析：
 *     ① 卡死检测条件过于严格，需要同时满足多个条件
 *     ② stuckStartTime 在卡死状态持续时可能被错误重置
 *     ③ 卡死状态到混沌触发的链路中存在条件判断漏洞
 * 
 *   解决方案：
 *     ① 增加详细的串口调试日志，实时输出卡死检测状态
 *     ② 在 loop() 中添加兜底保护：检测到 STATE_STUCK 超时时强制触发混沌
 *     ③ 降低卡死清除门槛，避免状态抖动导致 stuckStartTime 重置
 *     ④ startChaos() 增加醒目日志输出，便于确认触发
 * 
 * 
 *   改进2：Web页面重构 - 状态矩阵 + 实时监控
 *   ─────────────────────────────────────────────
 *   布局变更：
 *     【顶部区域 - 全宽显示】
 *     ├── 代际 / 个体 / 新奇度 / 存活时间
 *     ├── 状态矩阵卡片 (IDLE/WALKING/STUCK/CHAOS/ESCAPING 高亮)
 *     ├── 混沌噪声值 (GPIO1 实时读数)
 *     ├── 存储报警 (SPIFFS剩余空间，低于阈值变色)
 *     └── 一键下载全部文件 (新增)
 * 
 *     【下部区域 - 原有功能整合】
 *     ├── 实时状态 (传感器、编码器、电机)
 *     ├── 实验控制 (启动/停止/下一个)
 *     ├── 电机控制 (启用/禁用/方向)
 *     └── 数据导出 (历史/混沌/种群/基因/帧日志/个体)
 * 
 * 
 *   新增Web接口：
 *     /download/all - 一键导出所有数据文件
 * 
 *   扩展 /status 接口：
 *     - noiseValue: GPIO1 噪声实时读数
 *     - freeSpace: SPIFFS 剩余空间 (bytes)
 *     - freeSpaceKB: SPIFFS 剩余空间 (KB)
 *     - chaosTriggerCount: 混沌触发次数
 * 
 * 
 *   【状态转换图】v9.9
 *   ──────────────────────────────────────────────────────
 ...
 *   【混沌触发条件】v9.9
 *   ──────────────────────────────────────────────────────
 *   条件1: 差值卡死 - diff > 30 且持续增大，双轮都在转
 *   条件2: 单轮卡死 - 一轮转(>20)，另一轮停(<3)
 *   条件3: 大差值兜底 - diff > 60，任意轮在转
 *   条件4: 滑动窗口 - 10帧中至少7帧差值持续增大 (★ v9.8)
 *   兜底: STATE_STUCK 持续超过 CHAOS_TIMEOUT_MS (★ v9.9)
 * 
 * ================================================================
 */
...
#define PIN_NOISE_SOURCE    1       // ★ GPIO1 是噪声源
...
#define SPIFFS_MAX_INDIVIDUALS    2
#define INCREMENTAL_SAVE_INTERVAL 500

// ================================================================
// 固件版本
// ================================================================
#define FIRMWARE_VERSION "v9.9-ChaosFix+WebRefactor"
...
#define CHAOS_FORCE_TIMEOUT_MS  5000
...
struct FrameLogEntry {
    ...
    uint8_t  state : 3;          // ★ MotorState 枚举值
    uint8_t  reserved : 2;
};
...
public:
    ...
    static uint16_t readNoise() { return analogRead(PIN_NOISE_SOURCE); }  // ★ GPIO1
};
...
private:
    ...
    static const uint16_t FILE_VERSION = 0x0004;
...
public:
    ...
    static String extractFrames(int startFrame, int endFrame) {
        ...
        for (int i = 0; i < framesToExtract; i++) {
            ...
            const char* stateNames[] = {"IDLE", "WALK", "STUCK", "CHAOS", "ESCAP"};
...
private:
    ...
    static const int MAX_INDIVIDUALS_PER_GEN = SPIFFS_MAX_INDIVIDUALS;
    ...
    static void deleteGeneration(uint32_t gen) {
        ...
        while (File f = root.openNextFile()) {
            String name = String(f.name());
            bool shouldDelete = false;
            if (name.startsWith("/pop_gen_" + String(gen) + ".")) shouldDelete = true;
            if (name.startsWith("/gen_" + String(gen) + "_")) shouldDelete = true;
            if (name.startsWith("/frm_" + String(gen) + "_")) shouldDelete = true;
            if (name.startsWith("/chaos_g" + String(gen) + "_")) shouldDelete = true;
            if (shouldDelete) {
                f.close();
                SPIFFS.remove(name);
                deleted++;
...
public:
    ...
    static bool ensureSpace(size_t requiredBytes) {
        ...
        auto gens = getStoredGenerations();
        while (gens.size() >= MAX_GENERATIONS) {
            uint32_t oldest = gens[0];
    ...
    static bool saveFrameLog(uint32_t generation, int individual, 
                             const FrameLogEntry* log, int count) {
        if (!RobustStorage::isReady() || count == 0) return false;
        
        if (savedCount[generation % 100] >= MAX_INDIVIDUALS_PER_GEN) {
            Logger::logf("⏭️ Skip frame gen=%lu id=%d (Top%d only)",
                         generation, individual, MAX_INDIVIDUALS_PER_GEN);
            return false;
        }
    ...
    static bool incrementalSave(uint32_t generation, int individual,
        ...
        return saveFrameLog(generation, individual, log + startIdx, INCREMENTAL_SAVE_INTERVAL);
    }
...
private:
    ...
    static bool escapeActiveCompat;
    
    // ★ 滑动窗口卡死检测 ★
    static int32_t diffHistory[STUCK_WINDOW_SIZE];
    ...
    static void setMotorState(MotorState newState) {
        ...
        const char* stateNames[] = {"IDLE", "WALKING", "STUCK", "CHAOS", "ESCAPING"};
        Serial.printf("[STATE] → %s\n", stateNames[newState]);
    }

    static bool detectStuckWithWindow(int32_t diff) {
        diffHistory[historyIndex] = diff;
        historyIndex = (historyIndex + 1) % STUCK_WINDOW_SIZE;
        if (historyCount < STUCK_WINDOW_SIZE) historyCount++;
        
        if (historyCount < STUCK_WINDOW_SIZE) return false;
        
        int increasingCount = 0;
        for (int i = 0; i < STUCK_WINDOW_SIZE - 1; i++) {
            int curr = (historyIndex - 1 - i + STUCK_WINDOW_SIZE) % STUCK_WINDOW_SIZE;
            int prev = (historyIndex - 2 - i + STUCK_WINDOW_SIZE) % STUCK_WINDOW_SIZE;
            if (diffHistory[curr] > diffHistory[prev] + ENCODER_DIFF_MIN) {
                increasingCount++;
            }
        }
...
public:
    ...
    static bool isEscapeActive() { return motorState == STATE_ESCAPING || escapeActiveCompat; }
    ...
    static void setEscapeActive(bool val) { 
        if (val) setMotorState(STATE_ESCAPING);
        else if (motorState == STATE_ESCAPING) setMotorState(STATE_IDLE);
        escapeActiveCompat = val; 
    }
    ...
    static void init() {
        ...
        ledcWrite(LEDC_CHANNEL_RIGHT, 0);
        stopMotors();
        Logger::log("Motor Controller initialized (v9.9)");
    }
    ...
    static void updateChaos() {
        ...
        if (millis() - chaosStartTime > CHAOS_FORCE_TIMEOUT_MS) {
            Serial.println("[CHAOS] ⚠️ Timeout! No movement for 5s, force exit.");
    ...
    static void update(const Gene& gene) {
        ...
        if (isStuckCondition && motorState != STATE_CHAOS && motorState != STATE_ESCAPING) {
            ...
            if (motorState == STATE_STUCK) {
                ...
                if (shouldClear) {
                    ...
                    Serial.printf("[STUCK] ⏳ Still stuck, waiting... diff=%ld, elapsed=%lu\n", 
                                  diff, millis() - stuckStartTime);
                }
            }
        }
        lastDiff = diff;
        ...
        if (motorState == STATE_STUCK) {
            ...
            if (stuckDuration % 500 < 20) {  // 每500ms输出一次
                Serial.printf("[CHAOS] ⏳ Stuck duration: %lu / %d ms\n", stuckDuration, CHAOS_TIMEOUT_MS);
            }
            if (stuckDuration >= CHAOS_TIMEOUT_MS) {
                Serial.printf("[CHAOS] 🚀 TRIGGERING! duration=%lu\n", stuckDuration);
                startChaos();
                logFrame();
                return;
            }
            ...
            for (int i = 0; i < gene.ruleCount; i++) {
                if (gene.evaluateCondition(i, leftSensorRaw, rightSensorRaw, dist, elapsed)) {
                    ...
                    if (r.condType == COND_STUCK) {
                        setMotorState(STATE_ESCAPING);
                        escapeActiveCompat = true;
                        Logger::log("Escape rule triggered!");
                    }
                    if (r.condType == COND_ESCAPE) {
                        if (motorState == STATE_ESCAPING) setMotorState(STATE_IDLE);
                        escapeActiveCompat = false;
...
bool MotorController::escapeActiveCompat = false;

int32_t MotorController::diffHistory[STUCK_WINDOW_SIZE] = {0};
...
h3{color:#8b949e;font-size:12px;margin:5px 0;}
.card{background:#161b22;border:1px solid #30363d;border-radius:8px;padding:15px;margin:10px 0;}
.card-top{background:#161b22;border:1px solid #30363d;border-radius:8px;padding:15px;margin:10px 0;}
.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;}
.grid-3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;}
.status{font-size:12px;line-height:1.8;font-family:monospace;}
.val{color:#58a6ff;}.high{color:#3fb950;}.warn{color:#d29922;}.danger{color:#f85149;}

.state-matrix{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0;}
.state-box{padding:8px 16px;border-radius:6px;border:2px solid #30363d;font-size:12px;font-weight:bold;transition:all 0.3s;color:#8b949e;background:transparent;}
.state-box.active{transform:scale(1.05);}
...
.state-escaping.active{background:#58a6ff;border-color:#58a6ff;color:#0d1117;}

/* 非活跃状态保持灰色边框 */
.state-idle{border-color:#30363d;color:#8b949e;}
.state-walking{border-color:#30363d;color:#8b949e;}
.state-stuck{border-color:#30363d;color:#8b949e;}
.state-chaos{border-color:#30363d;color:#8b949e;}
.state-escaping{border-color:#30363d;color:#8b949e;}

@keyframes blink{0%,100%{opacity:1;}50%{opacity:0.5;}}

.storage-warning{background:#1f2937;border-left:4px solid #3fb950;padding:8px 12px;margin:8px 0;font-size:12px;}
.storage-warning.danger{border-left-color:#f85149;background:#2d1b1b;}
...
button.all-download{background:#7c3aed;border-color:#a78bfa;color:#fff;font-size:14px;padding:10px 24px;}
...
input{background:#0d1117;border:1px solid #30363d;color:#c9d1d9;padding:6px;border-radius:4px;width:80px;}
@media(max-width:600px){.grid,.grid-3{grid-template-columns:1fr;}.download-grid{grid-template-columns:1fr 1fr;}}
</style>
</head><body>
<h1>🚗 OEE Test <span style="font-size:14px;color:#8b949e;">V9.9</span></h1>

<!-- ★★★ 顶部状态区 ★★★ -->
<div class="card-top">
    <div class="grid">
        <div>
            <span style="font-size:14px;color:#8b949e;">代际</span>
            <span id="generation" class="val" style="font-size:20px;">-</span>
            <span style="font-size:14px;color:#8b949e;margin-left:15px;">个体</span>
            <span id="individual" class="val" style="font-size:20px;">-</span>
        </div>
        <div style="text-align:right;">
            <span style="font-size:14px;color:#8b949e;">新奇度</span>
            <span id="novelty" class="high" style="font-size:18px;">-</span>
            <span style="font-size:14px;color:#8b949e;margin-left:15px;">存活</span>
            <span id="survival" class="val" style="font-size:18px;">-</span>
        </div>
    </div>
    ...
    <div class="state-matrix" id="stateMatrix">
        ...
        <div class="state-box state-walking" data-state="1">🚶 WALKING</div>
        <div class="state-box state-stuck" data-state="2">⚠️ STUCK</div>
        <div class="state-box state-chaos" data-state="3">🔥 CHAOS</div>
        <div class="state-box state-escaping" data-state="4">🔄 ESCAPING</div>
    </div>
    
    <!-- ★ 混沌噪声 + 存储报警 + 一键下载 ★ -->
    <div class="grid-3">
        <div>
            <span style="font-size:12px;color:#8b949e;">🌪️ 混沌噪声 (GPIO1)</span>
            <span id="noiseValue" class="val" style="font-size:18px;display:block;">-</span>
            <span id="chaosCount" style="font-size:11px;color:#8b949e;">触发: 0次</span>
        </div>
        <div>
            <span style="font-size:12px;color:#8b949e;">💾 存储空间</span>
            <span id="freeSpace" class="val" style="font-size:18px;display:block;">-</span>
            <div id="storageWarning" class="storage-warning">✅ 空间充足</div>
        </div>
        <div style="display:flex;align-items:center;justify-content:flex-end;">
            <button class="all-download" onclick="downloadFile('/download/all')">📦 一键下载全部</button>
        </div>
    </div>
</div>
...
<div class="card">
    <h2>📊 实时状态</h2>
    <div class="grid">
        <div class="status" id="statusDetail">加载中...</div>
        <div class="status" id="sensorStatus">传感器: 加载中...</div>
    </div>
</div>
...
<div class="card">
    ...
    <div class='download-grid'>
        ...
        <button class='all-download' onclick="downloadFile('/download/all')">📦 全部</button>
    </div>
...
<script>
const stateNames = ['IDLE', 'WALKING', 'STUCK', 'CHAOS', 'ESCAPING'];

function api(url){fetch(url).then(r=>r.json()).catch(e=>console.error(e));}

function motorCtrl(a){
    fetch('/motor?action='+a).then(r=>r.json())
    .then(d=>{document.getElementById('motorStatus').innerHTML='电机: '+d.status;});
}
...
fetch('/status').then(r=>r.json()).then(d=>{
    document.getElementById('generation').textContent = d.generation;
    document.getElementById('individual').textContent = d.individual + 1 + '/' + d.population;
    document.getElementById('novelty').textContent = d.novelty.toFixed(4);
    ...
    document.getElementById('noiseValue').textContent = d.noiseValue;
    document.getElementById('chaosCount').textContent = '触发: ' + d.chaosTriggerCount + '次';
    
    var freeKB = d.freeSpaceKB || 0;
    document.getElementById('freeSpace').textContent = freeKB + ' KB';
    var warnEl = document.getElementById('storageWarning');
    if(freeKB < 50) {
        warnEl.className = 'storage-warning danger';
        warnEl.innerHTML = '⚠️ 存储空间严重不足! (' + freeKB + ' KB)';
    } else if(freeKB < 200) {
        warnEl.className = 'storage-warning';
        warnEl.style.borderLeftColor = '#d29922';
        warnEl.innerHTML = '⚠️ 存储空间偏低 (' + freeKB + ' KB)';
    } else {
        warnEl.className = 'storage-warning';
        warnEl.style.borderLeftColor = '#3fb950';
        warnEl.innerHTML = '✅ 空间充足 (' + freeKB + ' KB)';
    }
    
    document.getElementById('statusDetail').innerHTML =
        '状态: <span class='+ (d.stateCode===3?'warn':d.stateCode===2?'warn':'high') + '>' + d.state + '</span><br>' +
        '规则数: <span class=val>' + d.ruleCount + '</span><br>' +
        '距离: <span class=val>' + d.distance + '</span> ticks<br>' +
        '电机: ' + (d.motorEnabled?'<span class=high>✅ 已启用</span>':'<span class=warn>❌ 已禁用</span>') +
        '<br>动作: ' + (d.actionInProgress?'<span class=warn>执行中</span>':'<span class=val>空闲</span>');
    
    document.getElementById('sensorStatus').innerHTML =
        '左传感器: <span class=val>' + d.sensorLeft + '</span><br>' +
        '右传感器: <span class=val>' + d.sensorRight + '</span><br>' +
        '左编码器: <span class=val>' + d.leftTicks + '</span><br>' +
        '右编码器: <span class=val>' + d.rightTicks + '</span><br>' +
        '编码器差: <span class=' + (d.encoderDiff>50?'warn':'val') + '>' + d.encoderDiff + '</span>';
...
public:
    static void init() {
        ...
        server.on("/ramlog", handleRamLog);
        server.on("/ramlog/download", handleRamLogDownload);
        server.on("/storage/status", handleStorageStatus);
        
        server.onNotFound([](){ server.send(404, "text/plain", "Not Found"); });
        server.begin();
        Logger::log("Web server ready (v9.9)");
    }
    ...
    static void handleStatus() {
        ...
        if (RobustStorage::isReady()) {
            ...
            if (root) {
                ...
                root.close();
                freeSpace = 1572864 - used;
            }
        }
        
        uint16_t noiseValue = SensorCalibration::readNoise();
        ...
        json += "\"freeSpace\":" + String((int)freeSpace) + ",";
        ...
        json += "\"chaosTriggerCount\":" + String(MotorController::getTestChaosTriggerCount()) + ",";
        json += "\"noiseValue\":" + String(noiseValue);
        json += "}";
    ...
    static void handleRamLog() {
        if (server.hasArg("start") && server.hasArg("end")) {
            int start = server.arg("start").toInt();
            int end = server.arg("end").toInt();
            String result = RAMLogBuffer::extractFrames(start, end);
            server.send(200, "text/plain", result);
        } else {
            int count = RAMLogBuffer::getFrameCount();
            int start = max(0, count - 100);
            int end = count - 1;
            String result = RAMLogBuffer::extractFrames(start, end);
            server.send(200, "text/plain", result);
        }
    ...
    static void handleRamLogDownload() {
        if (!server.hasArg("start") || !server.hasArg("end")) {
            server.send(400, "text/plain", "Need start and end parameters");
            return;
        }
        int start = server.arg("start").toInt();
        int end = server.arg("end").toInt();
        ...
        if (start > end) { server.send(400, "text/plain", "Invalid range"); return; }
        
        size_t size = (end - start + 1) * sizeof(CompressedFrameEntry);
        uint8_t* buffer = (uint8_t*)malloc(size);
        if (!buffer) { server.send(500, "text/plain", "Memory allocation failed"); return; }
        
        size_t actualSize = RAMLogBuffer::extractBinary(buffer, size, start, end);
        if (actualSize == 0) {
            free(buffer);
        ...
        server.sendHeader("Content-Disposition", "attachment; filename=ram_log_" + 
                          String(RAMLogBuffer::getGeneration()) + "_i" + 
                          String(RAMLogBuffer::getIndividual()) + ".bin");
        server.send(200, "application/octet-stream", String((char*)buffer, actualSize));
        free(buffer);
    }

    static void handleStorageStatus() {
        String status = RollingStorage::getStorageStatus();
    ...
    static void handleDownloadAll() {
        ...
        String allData = "=== OEE Test V9.9 完整数据导出 ===\n";
        allData += "导出时间: " + String(millis()) + "\n\n";
        ...
        if (root) {
            while (File f = root.openNextFile()) {
                ...
                if (name.startsWith("/chaos_") || name == "/chaos_history.csv") {
                    ...
                    if (file) { allData += file.readString(); file.close(); }
                }
                f.close();
            }
            root.close();
        ...
        if (root) {
            while (File f = root.openNextFile()) {
                ...
                if (name.startsWith("/pop_gen_") && name.endsWith(".bin")) {
                    allData += name + ": " + String(f.size()) + " bytes\n";
                }
                f.close();
            }
            root.close();
        ...
        if (root) {
            ...
            while (File f = root.openNextFile()) {
                ...
                if (name.startsWith("/frm_") && name.endsWith(".bin")) {
                    int frames = (f.size() - sizeof(FileHeader)) / sizeof(CompressedFrameEntry);
                    allData += name + ": " + String(frames) + " frames\n";
                    count++;
                }
                f.close();
            }
        ...
        server.sendHeader("Content-Disposition", "attachment; filename=all_data_export.txt");
...
void setup() {
    ...
    Logger::log("  Chaos Fix + Web Refactor");
    Logger::log("  State Machine: IDLE/WALKING/STUCK/CHAOS/ESCAPING");
    Logger::log("  Commands: ls, status, frame, pop, chaos, popall, ramlog, storage, help");
    Logger::log("========================================");
    ...
    Logger::log("  Serial: ls, status, frame, pop, chaos, popall, ramlog, storage, help");
    Logger::log("========================================");
}
...
void loop() {
    ...
    if (millis() - lastChaosCheck > 100) {
        ...
        if (MotorController::getMotorState() == STATE_STUCK) {
            uint32_t stuckDuration = MotorController::getStateDuration();
            if (stuckDuration > CHAOS_TIMEOUT_MS + 500) {
                Serial.printf("[CHAOS] 🔥 FORCE TRIGGER! stuckDuration=%lu\n", stuckDuration);
    ...
    if (Serial.available()) {
        ...
        if (cmd == "ls" || cmd == "list") {
            ...
            Serial.println("  ramlog <开始> <结束> - 提取RAM日志，例如: ramlog 0 100");
