 ...
 * 文件：OEETestFinal_v9919_Final_v4.ino
...
#define SPIFFS_MAX_GENERATIONS    5
...
// 固件版本
// ================================================================
#define FIRMWARE_VERSION "v9.19-Final"
...
public:
    ...
    static void addChaosRecord(const ChaoticTestRecord& record) {
        ...
        chaosUnsavedCount++;
        totalSaved++;
        if (chaosPendingBuffer.length() > MAX_BUFFER_SIZE || chaosUnsavedCount >= 5) {
            flushChaosBuffer();
        }
    }
...
public:
    static void saveChaosSnapshotsToSPIFFS(uint32_t gen, int ind) {
        ...
        size_t totalSize = 1 + chaosSnapshotCount * sizeof(ChaosSnapshotEntry);
        uint8_t* buf = (uint8_t*)malloc(totalSize);
        if (buf) {
            buf[0] = chaosSnapshotCount;
            memcpy(buf + 1, chaosSnapshots, chaosSnapshotCount * sizeof(ChaosSnapshotEntry));
            FileUtils::atomicWrite(path, buf, totalSize);
            free(buf);
            Logger::logf("💾 Chaos snapshots backed up: %s (%d snapshots)", path.c_str(), chaosSnapshotCount);
        }
    }
...
public:
    ...
    static void update(const Gene& gene) {
        ...
        bool largeDiffStuck = (diff > encoderDiffThreshold * 2 && 
                               anySpinning);
        bool isStuckCondition = (windowStuck || diffStuck || oneWheelStuck || largeDiffStuck);
        
        static uint32_t lastDebugLog = 0;
        if (millis() - lastDebugLog > 1000) {
            lastDebugLog = millis();
            const char* stateNames[] = {"IDLE", "WALKING", "STUCK", "CHAOS"};
            Serial.printf("[DEBUG] diff=%ld, inc=%d, L=%d R=%d, stuck=%d, state=%s, stuckStart=%lu\n",
                          diff, diffIncreasing, currentSpeedL, currentSpeedR, 
                          isStuckCondition, stateNames[motorState], stuckStartTime);
        }
