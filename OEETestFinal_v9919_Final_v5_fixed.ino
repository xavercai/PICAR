 ...
 * 文件：OEETestFinal_v9919_Final_v5_fixed.ino
...
// 固件版本
// ================================================================
#define FIRMWARE_VERSION "v9.19-Final-v5"
...
struct ChaosSnapshotHeader {
    uint32_t magic;        // 魔数: 0x4348534E ('CHSN')
    uint16_t version;      // 版本: 0x0001
    uint16_t headerSize;   // 头大小: 12
    uint32_t crc32;        // CRC32校验（覆盖count + 所有快照数据）
    uint8_t  count;        // 快照数量
    uint8_t  reserved[3];  // 保留对齐
};

// === 序列化结构体尺寸锁定：防止编译器隐式 padding 漂移导致文件格式变化 ===
...
// ================================================================
// 前向声明：FileUtils 需要用到 RobustStorage
// ================================================================
class RobustStorage;

// ================================================================
// FileUtils 类 - 仅声明（实现移到 RobustStorage 之后）
// ================================================================
class FileUtils {
public:
...
private:
    ...
    static bool isValidHistoryValue(uint32_t ts, uint32_t gen, float novelty, 
        ...
        // 时间戳合理性: 应在 2020-01-01 ~ 2100-01-01 范围内
...
public:
    ...
    static void addRecord(const HistoryRecord& record, bool forceFlush = false) {
        ...
        unsavedCount++;
        totalSaved++;
        // P1改进: forceFlush=true时立即刷盘，或满足原有条件时刷盘
...
// ================================================================
// FileUtils 方法实现（必须在 RobustStorage 定义之后）
// ================================================================
...
struct Gene {
    ...
    void init() {
        ...
        chaosTimeoutMs       = random(500, 8001);
        chaosForceTimeoutMs  = random(1000, 15001);
        chaosRuleCount       = 0;
    ...
    void mutate(float mutationRate) {
        ...
        if (random(0, 1000) < mutationRate * 500) { chaosTimeoutMs       += random(-500, 501); chaosTimeoutMs       = constrain(chaosTimeoutMs,       500, 8000); }
        if (random(0, 1000) < mutationRate * 500) { chaosForceTimeoutMs  += random(-1000,1001);chaosForceTimeoutMs  = constrain(chaosForceTimeoutMs,  1000,15000); }
    }
...
private:
    ...
    static void deleteGeneration(uint32_t gen) {
        if (!RobustStorage::isReady()) return;
        // 修复：两段式删除——先收集文件名并关闭目录迭代器，再删除，避免遍历中删除导致跳过
        std::vector<String> toDelete;
...
public:
    ...
    static bool ensureSpace(size_t requiredBytes) {
        ...
        // 修复死锁：限制淘汰轮数，并在删除失败(代际数未下降)时立即退出，避免无限循环
        auto gens = getStoredGenerations();
...
class GeneStorage {
private:
    static const uint32_t MAX_GENERATIONS_PER_EXPERIMENT = 0xFFFFFFFF;  // 无限进化：实验永不"完成"，由滚动保留策略接管
...
public:
    ...
    static bool savePopulation(Gene* population, int size) {
        ...
        for (int i = 0; i < size; i++) {
            uint8_t rc = population[i].ruleCount;
            if (rc > MAX_RULES) rc = MAX_RULES;  // 防越界：ruleCount 异常时钳制到上限
            totalSize += 1 + rc * sizeof(BehaviorRule) + 12 + 24;
        }
        if (!RollingStorage::ensureSpace(totalSize + 512)) {
            Logger::log("❌ savePopulation: insufficient SPIFFS space");
            return false;
        }
        ...
        for (int i = 0; i < size; i++) {
            uint8_t rc = population[i].ruleCount;
            if (rc > MAX_RULES) rc = MAX_RULES;  // 与 totalSize 计算保持一致，防止越界读 rules[]
            *ptr = rc; ptr += 1;
            for (int j = 0; j < rc; j++) {
            ...
            memcpy(ptr, &population[i].obstacleThreshold, 2); ptr += 2;
            memcpy(ptr, &population[i].clearThreshold, 2); ptr += 2;
            memcpy(ptr, &population[i].encoderDiffThreshold, 2); ptr += 2;
            memcpy(ptr, &population[i].encoderDiffMin, 2); ptr += 2;
            memcpy(ptr, &population[i].wheelSpinThreshold, 2); ptr += 2;
            memcpy(ptr, &population[i].wheelStopThreshold, 2); ptr += 2;
            *ptr = population[i].stuckWindowSize; ptr += 1;
            memcpy(ptr, &population[i].chaosNoiseAmplifier, 2); ptr += 2;
            memcpy(ptr, &population[i].chaosMinPwm, 2); ptr += 2;
            memcpy(ptr, &population[i].chaosTimeoutMs, 2); ptr += 2;
            memcpy(ptr, &population[i].chaosForceTimeoutMs, 2); ptr += 2;
            *ptr = population[i].hasChaosRules ? 1 : 0; ptr += 1;
            *ptr = population[i].chaosRuleCount; ptr += 1;
            *ptr = population[i].chaosRulesStartIndex; ptr += 1;
        }
    ...
    static bool loadPopulation(uint32_t generation, Gene* population, int size) {
        if (!RobustStorage::isReady()) {
            Logger::logf("❌ loadPopulation: SPIFFS not ready");
            return false;
        }
        String path = String(POP_PREFIX) + String(generation) + ".bin";
        Logger::logf("📖 loadPopulation: trying %s", path.c_str());
        ...
        memcpy(&fileVersion, ptr, 2); ptr += 2;
        uint16_t popSize; memcpy(&popSize, ptr, 2); ptr += 2;
        if (popSize != size) { 
            Logger::logf("❌ loadPopulation: size mismatch: %d vs %d", popSize, size);
            free(buffer); 
            return false; 
        }
        ptr += 4;
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (population[i].ruleCount > MAX_RULES) {
                population[i].ruleCount = MAX_RULES;
            }
            ...
            if (fileVersion >= 0x0005) {
                memcpy(&population[i].obstacleThreshold, ptr, 2); ptr += 2;
                memcpy(&population[i].clearThreshold, ptr, 2); ptr += 2;
                memcpy(&population[i].encoderDiffThreshold, ptr, 2); ptr += 2;
                memcpy(&population[i].encoderDiffMin, ptr, 2); ptr += 2;
                memcpy(&population[i].wheelSpinThreshold, ptr, 2); ptr += 2;
                memcpy(&population[i].wheelStopThreshold, ptr, 2); ptr += 2;
                population[i].stuckWindowSize = *ptr; ptr += 1;
                memcpy(&population[i].chaosNoiseAmplifier, ptr, 2); ptr += 2;
                memcpy(&population[i].chaosMinPwm, ptr, 2); ptr += 2;
                memcpy(&population[i].chaosTimeoutMs, ptr, 2); ptr += 2;
                memcpy(&population[i].chaosForceTimeoutMs, ptr, 2); ptr += 2;
            } else {
                population[i].obstacleThreshold    = 1500;
                population[i].clearThreshold       = 600;
                population[i].encoderDiffThreshold = 30;
                population[i].encoderDiffMin       = 5;
                population[i].wheelSpinThreshold   = 20;
                population[i].wheelStopThreshold   = 3;
                population[i].stuckWindowSize      = 10;
                population[i].chaosNoiseAmplifier  = 180;
                population[i].chaosMinPwm          = 20;
                population[i].chaosTimeoutMs       = 3000;
                population[i].chaosForceTimeoutMs  = 5000;
            }
            if (fileVersion >= 0x0006) {
                population[i].hasChaosRules        = (*ptr == 1); ptr += 1;
                population[i].chaosRuleCount       = *ptr; ptr += 1;
                population[i].chaosRulesStartIndex = *ptr; ptr += 1;
            } else {
                population[i].hasChaosRules        = false;
                population[i].chaosRuleCount       = 0;
                population[i].chaosRulesStartIndex = 0;
            }
...
public:
    ...
    bool addIfNovel(const BehaviorDescriptor& desc) {
        ...
        if (archiveSize < NOVELTY_ARCHIVE_MAX) {
            ...
            // 满：丢弃最旧（索引0），整体左移，末尾追加新行为，保持 archiveSize=MAX
...
private:
    ...
    static int stuckFrameCount;  // 累计卡住帧数(新增)
...
public:
    ...
    static void saveChaosSnapshotsToSPIFFS(uint32_t gen, int ind) {
        ...
        // ★★★ P2改进: 空间预检查 ★★★
        // R4修复: totalSize = header(16) + payload(count + snapshots)
        ...
        if (buf) {
            ...
            // R4修复: 写入payload (count + snapshots)
...
public:
    ...
    static void init() {
        ...
        historyIndex = 0;
        historyCount = 0;
        stuckFrameCount = 0;  // 累计卡住帧数初始化(新增)
    ...
    static void updateChaos() {
        ...
        if (millis() - chaosStartTime > chaosForceTimeoutMs) {
            flushChaosSnapshot();
            Serial.println("[CHAOS] ⚠️ Timeout! No movement for timeout, force exit.");
            deathFlag = true;
    ...
    static void update(const Gene& gene) {
        ...
        // ★★★ 修复：增强单轮卡住检测 ★★★
        // 原逻辑：要求一侧"转"且另一侧"停"(过于严格)
        // 新逻辑：放宽条件，允许两轮都转动但速度差过大时也判定为卡住
        bool oneWheelStuck = ((leftSpinning && rightStopped) || 
                              (rightSpinning && leftStopped));
        // 新增：差速卡住 - 两轮都转动但差速持续存在
        bool diffStuckOneWheel = (diff > encoderDiffThreshold && 
                                  (currentSpeedL > wheelStopThreshold * 2 || 
                                   currentSpeedR > wheelStopThreshold * 2));
        
        bool largeDiffStuck = (diff > encoderDiffThreshold * 2 && 
                               anySpinning);
        
        // ★★★ 核心修复：累计帧数检测 ★★★
        // 如果满足卡住条件之一，累计帧数；否则递减
        bool potentialStuck = (windowStuck || diffStuck || oneWheelStuck || 
                               largeDiffStuck || diffStuckOneWheel);
        if (potentialStuck && motorState != STATE_CHAOS) {
            stuckFrameCount++;
        } else {
            // 当不在STUCK状态时，重置帧计数器
        ...
        // 当累计帧数超过阈值(约2秒@100ms周期)时，也判定为卡住
        #define STUCK_FRAME_THRESHOLD 20  // 连续20帧(约2秒)满足条件则触发
        bool frameAccumulatedStuck = (stuckFrameCount >= STUCK_FRAME_THRESHOLD);
        
        bool isStuckCondition = (windowStuck || diffStuck || oneWheelStuck || 
                                 largeDiffStuck || diffStuckOneWheel || frameAccumulatedStuck);
        
        static uint32_t lastDebugLog = 0;
        if (millis() - lastDebugLog > 1000) {
            lastDebugLog = millis();
            const char* stateNames[] = {"IDLE", "WALKING", "STUCK", "CHAOS"};
            Serial.printf("[DEBUG] diff=%ld, inc=%d, L=%d R=%d, stuck=%d, frames=%d, state=%s\n",
                          diff, diffIncreasing, currentSpeedL, currentSpeedR, 
                          isStuckCondition, stuckFrameCount, stateNames[motorState]);
        }
        ...
        if (isStuckCondition && motorState != STATE_CHAOS) {
            if (motorState != STATE_STUCK) {
                ...
                stuckFrameCount = 0;  // 进入STUCK后重置帧计数
                Serial.printf("[STUCK] ✅ Detected! diff=%ld, L=%d, R=%d, time=%lu\n", 
                              diff, currentSpeedL, currentSpeedR, stuckStartTime);
            }
        } else {
            if (motorState == STATE_STUCK) {
                ...
                if (shouldClear) {
                    clearStuckState();
                    stuckFrameCount = 0;
                    Serial.printf("[STUCK] Cleared (diff=%ld)\n", diff);
                }
            }
        }
        lastDiff = diff;
...
int MotorController::stuckFrameCount = 0;  // 累计卡住帧数(新增)
...
private:
    ...
    void initImpl() {
        ...
        if (!archive.load()) archive.init();  // 新颖度存档跨重启保留，无存档才新建
    ...
    void startCurrentTestImpl() {
        ...
        Logger::logf("▶️ Testing individual %d/%d (gen %lu) [rules=%d]",
    ...
    void endCurrentTestImpl() {
        ...
        if (MotorController::getFrameLogCount() == 0) {
            ...
            if (added) archive.save();  // 新行为入档后立即落盘，防断电丢失
        }
        ...
        // ★★★ 关键点: 测试结束时同步刷盘，防止断电丢失进化历史 ★★★
        RobustStorage::addRecord(record, true);
        RobustStorage::forceSave();  // 双重保障：确保所有缓冲数据立即落盘
        
        Logger::logf("⏹️ Individual %d done: novelty=%.4f rules=%d %s",
        ...
        // ★★★ 关键点: 测试结束时同步刷盘混沌日志，防止断电丢失 ★★★
        RobustStorage::addChaosRecord(chaosRecord, true);
        
        Logger::logf("📊 End of individual %d: geneOk=%d, frameOk=%d, chaosOk=%d",
...
<body>
<div class="header">
<h1>🚗 OEE <span>v9.19</span><span class="version-badge">进化</span></h1>
...
<div class="stat-item"><div class="stat-label">混沌触发</div><div class="stat-value warn" id="chaosCnt">-</div></div>
</div>
...
<div class="sensor-item"><span class="label">右编码器</span><span class="value" id="encR">-</span></div>
 <!-- ★★★ 添加噪声显示 ★★★ -->
    <div class="sensor-item" style="grid-column: span 2; background: #1a1a2e;">
        <span class="label">🎲 物理噪声 (GPIO1)</span>
        <span class="value" id="noise" style="color: #a78bfa;">-</span>
    </div>
    <div class="sensor-item" style="grid-column: span 2; background: #1a1a2e;">
        <span class="label">📢 噪声放大器</span>
        <span class="value" id="noiseAmp" style="color: #fbbf24;">-</span>
</div>
</div>
<div class="card">
...
document.getElementById('noise').textContent = d.noiseRaw || '0';
document.getElementById('noiseAmp').textContent = d.noiseAmplifier || '0';
...
public:
    ...
    static void handleStatus() {
        ...
        json += "\"noiseAmplifier\":" + String(MotorController::getChaosNoiseAmplifier());
        json += "}";
...
void setup() {
    ...
    Logger::log("  Data Persistence FIXED - All files now save");
...
void loop() {
    ...
    if (Serial.available()) {
        ...
        if (cmd == "ls" || cmd == "list") {
            ...
            Serial.println("========================================");
            Serial.println("可用命令 (v9.19-Final):");
...
void parseAllIndividuals(int gen) {
    ...
    Serial.println("========================================");
    Serial.println("ID | 规则数 | 新奇度 | 混沌次数 | 左轮速度 | 右轮速度 | 评价");
    Serial.println("---|--------|--------|---------|---------|---------|------");
    ...
    for (int id = 0; id < POPULATION_SIZE; id++) {
        ...
        Serial.printf("%2d | %6d | %6.3f | %7d | %7.1f | %7.1f | %s\n",
            id, ruleCount, novelty, chaosCount, avgL, avgR, eval.c_str());
    }
    Serial.println("========================================");
}
