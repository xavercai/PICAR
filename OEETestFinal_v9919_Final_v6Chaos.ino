...
public:
    ...
    // ============================================================
    // ★★★ 简化版混沌触发：编码器差值 > 20 维持 1 秒 ★★★
    // ============================================================
    static uint32_t diffExceedStartTime = 0;
    static bool waitingForTrigger = false;
    ...
    // 检测编码器差值是否超过阈值 (20)
    bool diffExceeded = (diff > 20);
    
    if (diffExceeded) {
        // 第一次超过阈值，记录开始时间
        if (!waitingForTrigger) {
            diffExceedStartTime = millis();
            waitingForTrigger = true;
            Serial.printf("[CHAOS] ⏱️ Diff exceeded 20: diff=%ld, start timer (waiting 1s)\n", diff);
        }
        
        // 检查是否维持了1秒
        uint32_t elapsed = millis() - diffExceedStartTime;
        if (elapsed >= 1000) {
            Serial.printf("[CHAOS] 🚀 TRIGGERED! Diff=%ld maintained for %lu ms\n", diff, elapsed);
            waitingForTrigger = false;
            diffExceedStartTime = 0;
            accumulatedStuckTime = 0;
            startChaos();
            logFrame();
            return;
        ...
        // 差值回到阈值以下，重置计时
        if (waitingForTrigger) {
            Serial.printf("[CHAOS] ⏹️ Diff returned to %ld (below 20), reset timer\n", diff);
            waitingForTrigger = false;
            diffExceedStartTime = 0;
        }
    }
