...
// 固件版本
// ================================================================
#define FIRMWARE_VERSION "v9.21-ChaosTuned+Storage"
...
// ★★★ v9.21: 混沌触发时间尺度参数 ★★★
// ================================================================
// chaosTimeoutMs: 卡死累计时间达到此值触发混沌
//   v9.19: random(500, 8001)  → 平均 ~4000ms
//   v9.21: random(800, 1801)  → 平均 ~1300ms (保障3轮混沌)
//
// chaosForceTimeoutMs: 混沌持续最长时间，超时强制退出
//   v9.19: random(1000, 15001) → 平均 ~8000ms
//   v9.21: random(3000, 8001)  → 平均 ~5500ms (给混沌充分脱困时间)
//   ★★★ A1/A2: 最小值 1500→3000, 最大值 3500→8000 ★★★
#define CHAOS_TIMEOUT_MIN       800
#define CHAOS_TIMEOUT_MAX       1800
#define CHAOS_FORCE_TIMEOUT_MIN 3000   // ★★★ A1: 原 1500 ★★★
#define CHAOS_FORCE_TIMEOUT_MAX 8000   // ★★★ A2: 原 3500 ★★★

// ================================================================
// ★★★ v9.21: 检测机制参数 ★★★
// ================================================================
#define STUCK_CLEAR_DEBOUNCE_MS     150  // P2: 清除防抖时间
#define BOUNCE_WINDOW_MS            500  // P3: 反复进出检测窗口
#define BOUNCE_THRESHOLD            3    // P3: 窗口内反复次数阈值
#define NO_PROGRESS_TIMEOUT_MS      1000 // P4: 零进展检测超时
#define SPEED_ASYMMETRY_RATIO       2.5f // P1: 速度不对称比例阈值
...
// ★★★ M1: 保留自 v6 的 R4修复 - 混沌快照文件头（含CRC校验）★★★
...
// ================================================================
// FileUtils 类 - 仅声明（必须在 RobustStorage 使用它之前声明）
// ================================================================
class FileUtils {
public:
...
// ================================================================
// FileUtils 方法实现（必须在 RobustStorage 完整定义之后）
// ================================================================
...
struct Gene {
    ...
    void init() {
        ...
        // ★★★ A3: chaosForceTimeoutMs 范围 3000~8000 ★★★
    ...
    void mutate(float mutationRate) {
        ...
        // ★★★ A4: chaosForceTimeoutMs 钳制范围 3000~8000 ★★★
        if (random(0, 1000) < mutationRate * 500) {
...
<body>
<div class="header">
<h1>🚗 OEE <span>v9.21</span><span class="version-badge">3轮混沌+存储</span></h1>
...
void setup() {
    ...
    Logger::log("  🔍 P1: 速度不对称检测 (ratio>2.5)");
    Logger::log("  🛡️ P2: 清除防抖 (150ms)");
    Logger::log("  ⚡ P3: 自适应阈值保护");
    Logger::log("  🛟 P4: 零进展检测 (1000ms)");
    Logger::log("  💾 存储: CRC校验 + CSV校验 + forceFlush");
    Logger::log("  ⏱️  chaosForceTimeout=3000~8000ms (充分脱困时间)");
...
void loop() {
    ...
    if (Serial.available()) {
        ...
        if (cmd == "ls" || cmd == "list") {
            ...
            Serial.println("========================================");
            Serial.println("可用命令 (v9.21-ChaosTuned+Storage):");
