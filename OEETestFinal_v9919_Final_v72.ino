...
// 固件版本
// ================================================================
#define FIRMWARE_VERSION "v9.21-RobustFixed"
...
// ★★★ v9.21: 混沌触发时间尺度参数 ★★★
// ================================================================
#define CHAOS_TIMEOUT_MIN       800
#define CHAOS_TIMEOUT_MAX       1800
...
struct Gene {
    ...
    void init() {
        ...
        chaosForceTimeoutMs  = random(3000, 8001);
        chaosRuleCount       = 0;
    ...
    void mutate(float mutationRate) {
        ...
        if (random(0, 1000) < mutationRate * 500) {
            ...
            chaosForceTimeoutMs = constrain(chaosForceTimeoutMs, 3000, 8000);
        }
    }
...
public:
    ...
    static bool ensureSpace(size_t requiredBytes) {
        ...
        if (free < requiredBytes + freeSpaceThreshold) {
            ...
            if (!gens.empty()) {
                uint32_t oldest = gens[0];
                Logger::logf("⚠️ Space low (%d bytes), deleting gen %lu", free, oldest);
        ...
        while ((int)gens.size() >= MAX_GENERATIONS && maxRounds-- > 0) {
            ...
            uint32_t oldest = gens[0];
            Logger::logf("🔄 Too many gens (%d), deleting %lu", gens.size(), oldest);
            deleteGeneration(oldest);
...
private:
    ...
    static size_t serializeIndividual(const Gene& g, uint8_t* buffer, size_t maxSize) {
        ...
        Logger::logf("  🔧 serializeIndividual: maxSize=%d, ruleCount=%d", maxSize, g.ruleCount);
        
        // 1. ruleCount (1)
        if (used + 1 > maxSize) { Logger::log("  ❌ serialize: ruleCount overflow"); return 0; }
        uint8_t rc = constrain(g.ruleCount, MIN_RULES, MAX_RULES);
        *ptr = rc; ptr++; used += 1;
        
        // 2. rules (N * sizeof(BehaviorRule))
        size_t rulesSize = rc * sizeof(BehaviorRule);
        if (used + rulesSize > maxSize) { Logger::log("  ❌ serialize: rules overflow"); return 0; }
        memcpy(ptr, g.rules, rulesSize);
        ptr += rulesSize; used += rulesSize;
        
        // 3. survival_time (4)
        if (used + 4 > maxSize) { Logger::log("  ❌ serialize: survival_time overflow"); return 0; }
        memcpy(ptr, &g.survival_time, 4);
        ptr += 4; used += 4;
        
        // 4. distance_ticks (4)
        if (used + 4 > maxSize) { Logger::log("  ❌ serialize: distance_ticks overflow"); return 0; }
        memcpy(ptr, &g.distance_ticks, 4);
        ptr += 4; used += 4;
        
        // 5. noveltyScore (4)
        if (used + 4 > maxSize) { Logger::log("  ❌ serialize: noveltyScore overflow"); return 0; }
        memcpy(ptr, &g.noveltyScore, 4);
        ptr += 4; used += 4;
        
        // 6-11. 阈值参数 (2+2+2+2+2+2 = 12)
        if (used + 12 > maxSize) { Logger::log("  ❌ serialize: thresholds overflow"); return 0; }
        ...
        if (used + 1 > maxSize) { Logger::log("  ❌ serialize: stuckWindowSize overflow"); return 0; }
        *ptr = g.stuckWindowSize; ptr++; used += 1;
        
        // 13-16. chaos参数 (2+2+2+2 = 8)
        if (used + 8 > maxSize) { Logger::log("  ❌ serialize: chaos params overflow"); return 0; }
        ...
        if (used + 3 > maxSize) { Logger::log("  ❌ serialize: chaos rules info overflow"); return 0; }
        *ptr = g.hasChaosRules ? 1 : 0; ptr++; used += 1;
        *ptr = g.chaosRuleCount; ptr++; used += 1;
        *ptr = g.chaosRulesStartIndex; ptr++; used += 1;
        
        Logger::logf("  ✅ serializeIndividual: used=%d bytes", used);
        return used;
    }

    // ★★★ 修复1: 反序列化单个个体 - 带完整边界检查和版本兼容 ★★★
    static size_t deserializeIndividual(const uint8_t* buffer, size_t maxSize, Gene& g, uint16_t fileVersion) {
        const uint8_t* ptr = buffer;
        size_t used = 0;
        
        Logger::logf("  🔧 deserializeIndividual: maxSize=%d, fileVersion=0x%04X", maxSize, fileVersion);
        
        // 1. ruleCount
        if (used + 1 > maxSize) { Logger::log("  ❌ deserialize: ruleCount overflow"); return 0; }
        g.ruleCount = *ptr; ptr++; used += 1;
        if (g.ruleCount > MAX_RULES) {
            Logger::logf("  ⚠️ deserialize: ruleCount %d clamped to %d", g.ruleCount, MAX_RULES);
            g.ruleCount = MAX_RULES;
        }
        
        // 2. rules
        size_t rulesSize = g.ruleCount * sizeof(BehaviorRule);
        if (used + rulesSize > maxSize) { Logger::log("  ❌ deserialize: rules overflow"); return 0; }
        memcpy(g.rules, ptr, rulesSize);
        ptr += rulesSize; used += rulesSize;
        
        // 3-5. survival, distance, novelty
        if (used + 12 > maxSize) { Logger::log("  ❌ deserialize: survival/distance/novelty overflow"); return 0; }
        ...
        if (fileVersion >= 0x0005) {
            // 6-11. 阈值参数 (12 bytes)
            if (used + 12 > maxSize) { Logger::log("  ❌ deserialize: thresholds overflow"); return 0; }
            ...
            // 12. stuckWindowSize (1)
            if (used + 1 > maxSize) { Logger::log("  ❌ deserialize: stuckWindowSize overflow"); return 0; }
            g.stuckWindowSize = *ptr; ptr++; used += 1;
            
            // 13-16. chaos参数 (8 bytes)
            if (used + 8 > maxSize) { Logger::log("  ❌ deserialize: chaos params overflow"); return 0; }
            ...
            if (fileVersion >= 0x0006) {
                // 17-19. chaos规则信息 (3 bytes)
                if (used + 3 > maxSize) { Logger::log("  ❌ deserialize: chaos rules info overflow"); return 0; }
                g.hasChaosRules = (*ptr == 1); ptr++; used += 1;
                g.chaosRuleCount = *ptr; ptr++; used += 1;
                g.chaosRulesStartIndex = *ptr; ptr++; used += 1;
                Logger::logf("  ✅ deserialize: chaosRules: has=%d, count=%d, start=%d",
                             g.hasChaosRules, g.chaosRuleCount, g.chaosRulesStartIndex);
            } else {
                Logger::log("  ℹ️ deserialize: version < 0x0006, using default chaos rules");
            ...
            // 旧版本默认值
            Logger::log("  ℹ️ deserialize: version < 0x0005, using default params");
        ...
        g.behavior.init();
        Logger::logf("  ✅ deserializeIndividual: used=%d bytes", used);
        return used;
    }
...
public:
    ...
    // ★★★ 修复1: 重写 savePopulation 使用 serializeIndividual ★★★
    static bool savePopulation(Gene* population, int size) {
        ...
        // 精确计算总大小
        size_t totalSize = 4 + 2 + 2 + 4 + 4; // header: magic, version, popSize, generation, expId
        Logger::logf("  Header size: %d bytes", 4+2+2+4+4);
        
        for (int i = 0; i < size; i++) {
            uint8_t rc = constrain(population[i].ruleCount, MIN_RULES, MAX_RULES);
            size_t indivSize = 1;                              // ruleCount
            indivSize += rc * sizeof(BehaviorRule);           // rules
            indivSize += 4 + 4 + 4;                           // survival, distance, novelty
            indivSize += 12 + 1 + 8 + 3;                      // 阈值 + stuckWindowSize + chaos参数 + chaos规则信息
            totalSize += indivSize;
            if (i < 3 || i >= size - 3) {
                Logger::logf("  Individual %d: rc=%d, size=%d, cumulative=%d", 
                             i, rc, indivSize, totalSize);
            } else if (i == 3) {
                Logger::log("  ... (skipping middle individuals for brevity)");
            }
        }
        ...
        Logger::logf("  Header: magic=0x%08X, version=0x%04X, popSize=%d, gen=%lu, expId=%lu",
                     magic, version, popSize, gen, expId);
        ...
        if (finalSize != totalSize) {
            Logger::logf("⚠️ savePopulation: size mismatch! expected=%d, actual=%d", totalSize, finalSize);
            totalSize = finalSize;
        }
        ...
        Logger::logf("💾 savePopulation gen=%lu -> %s %s (size=%d bytes)", 
                     currentGeneration, path.c_str(), success ? "✅ OK" : "❌ FAILED", totalSize);
        return success;
    }
    
    // ★★★ 修复1: 重写 loadPopulation 使用 deserializeIndividual 带完整性校验 ★★★
    static bool loadPopulation(uint32_t generation, Gene* population, int size) {
        ...
        if (magic != FILE_MAGIC) { 
            Logger::logf("❌ loadPopulation: invalid magic 0x%08X (expected 0x%08X)", magic, FILE_MAGIC);
            free(buffer); 
            return false; 
        }
        Logger::logf("  Magic: 0x%08X ✅", magic);
        ...
        // 读取 generation 和 expId (跳过)
        uint32_t fileGen, fileExpId;
        memcpy(&fileGen, ptr, 4); ptr += 4;
        memcpy(&fileExpId, ptr, 4); ptr += 4;
        Logger::logf("  fileGen=%lu, fileExpId=%lu", fileGen, fileExpId);
        
        // 检查文件大小是否足够
        size_t remaining = fileSize - (ptr - buffer);
        // 每个个体最小大小: 1(ruleCount) + MIN_RULES*14(rules) + 4+4+4 + 12+1+8+3
        size_t minPerIndiv = 1 + MIN_RULES * sizeof(BehaviorRule) + 4 + 4 + 4 + 12 + 1 + 8 + 3;
        size_t expectedMin = size * minPerIndiv;
        
        Logger::logf("  Remaining data: %d bytes, expected min: %d bytes", remaining, expectedMin);
        
        if (remaining < expectedMin) {
            Logger::logf("❌ loadPopulation: file too small! remaining=%d, need at least %d", 
                         remaining, expectedMin);
            free(buffer);
            return false;
        }
        ...
        // 逐个反序列化个体
        for (int i = 0; i < size; i++) {
            Logger::logf("  Deserializing individual %d/%d...", i+1, size);
            ...
            ptr += read;
            remaining -= read;
            Logger::logf("    ✅ Individual %d: remaining=%d bytes", i, remaining);
        }
        
        free(buffer);
        Logger::logf("✅ loadPopulation: loaded %d individuals from gen %lu (version 0x%04X)", 
                     size, generation, fileVersion);
        return true;
    }
    
    static bool saveIndividualRecord(uint32_t generation, int individual, const Gene& gene) {
        if (!RobustStorage::isReady()) {
            Logger::logf("❌ saveIndividualRecord: SPIFFS not ready");
            return false;
        }
        
        size_t estimatedSize = 512 + (size_t)gene.ruleCount * 64;
        if (!RollingStorage::ensureSpace(estimatedSize)) {
            Logger::logf("❌ saveIndividualRecord: insufficient space (need %d bytes)", estimatedSize);
            return false;
        }
        
        if (!experimentActive) {
            Logger::logf("⚠️ WARNING: experimentActive=false, but still saving individual record");
        }
    ...
    static bool saveChaosRecord(const ChaoticTestRecord& record) {
        if (!RobustStorage::isReady()) {
            Logger::logf("❌ saveChaosRecord: SPIFFS not ready");
            return false;
        }
        
        if (!experimentActive) {
            Logger::logf("⚠️ WARNING: experimentActive=false, but still saving chaos record");
        }
...
public:
    ...
    static void update(const Gene& gene) {
        ...
        if (isStuckCondition && motorState != STATE_CHAOS) {
            if (motorState != STATE_STUCK) {
                if (millis() - lastStuckClearTime < BOUNCE_WINDOW_MS) {
                    ...
                    if (stuckBounceCount > BOUNCE_THRESHOLD) {
                        savedEncoderDiffThreshold = encoderDiffThreshold;
                        encoderDiffThreshold = max(encoderDiffThreshold / 2, 5);
                        stuckBounceCount = 0;
                        Logger::logf("[STUCK] ⚠️ Bounce protection: threshold %d→%d", 
                                     savedEncoderDiffThreshold, encoderDiffThreshold);
                    }
                } else {
                    stuckBounceCount = 0;
            ...
            if (motorState != STATE_STUCK && encoderDiffThreshold != savedEncoderDiffThreshold) {
                encoderDiffThreshold = savedEncoderDiffThreshold;
            }
        }
        lastDiff = diff;
        ...
        if (motorState == STATE_STUCK) {
            ...
            if (totalStuckTime >= chaosTimeoutMs) {
                Serial.printf("[CHAOS] 🚀 TRIGGERING! totalStuckTime=%lu\n", totalStuckTime);
                accumulatedStuckTime = 0;
                startChaos();
                logFrame();
                return;
...
// ================================================================
// CarWebServer 类 - 修复 handleDownloadPopulation
// ================================================================
class CarWebServer {
private:
...
<title>OEE RobustFixed</title>
<style>
*{margin:0;padding:0;box-sizing:border-box;}
body{font-family:-apple-system,sans-serif;background:#0d1117;color:#e6edf3;padding:12px;max-width:480px;margin:0 auto;}
.header{background:#161b22;border:1px solid #30363d;border-radius:12px;padding:16px;margin-bottom:12px;}
.header h1{font-size:20px;color:#58a6ff;}
.version-badge{background:#1f2937;color:#58a6ff;font-size:10px;padding:2px 8px;border-radius:10px;}
.card{background:#161b22;border:1px solid #30363d;border-radius:10px;padding:14px;margin-bottom:10px;}
.card-title{font-size:11px;color:#8b949e;margin-bottom:8px;}
.stat-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;}
.stat-item{background:#0d1117;border-radius:6px;padding:8px;text-align:center;}
...
.btn{background:#21262d;color:#c9d1d9;border:1px solid #30363d;padding:8px 14px;border-radius:6px;cursor:pointer;font-size:12px;text-align:center;}
...
<body>
<div class="header"><h1>🚗 OEE <span class="version-badge">v9.21 RobustFixed</span></h1>
<div style="display:flex;justify-content:space-between;margin-top:6px;font-size:13px;">
<span>代 <span id="gen" class="blue">-</span></span>
<span>个体 <span id="ind" class="blue">-</span></span>
<span>新奇度 <span id="nov" class="high">-</span></span>
</div></div>
<div class="card"><div class="card-title">📊 实时数据</div>
<div class="stat-grid">
...
<div class="card"><div class="card-title">🎮 控制</div>
<div class="btn-group">
<button class="btn btn-primary" onclick="fetch('/evolution?action=start')">▶ 启动</button>
<button class="btn btn-danger" onclick="fetch('/evolution?action=stop')">⏹ 停止</button>
<button class="btn" onclick="fetch('/evolution?action=next')">⏭ 下一个</button>
<button class="btn" onclick="location.reload()">🔄 刷新</button>
</div></div>
<div class="card"><div class="card-title">📥 导出CSV</div>
<div class="btn-group">
<button class="btn btn-download" onclick="location.href='/download/history'">📋 历史</button>
<button class="btn btn-purple" onclick="location.href='/download/chaos'">🌪️ 混沌</button>
<button class="btn btn-download" onclick="location.href='/download/population'">🧬 种群</button>
</div></div>
<script>
...
public:
    ...
    // ★★★ 修复2: 重写 handleDownloadPopulation 使用正确的解析 ★★★
    static void handleDownloadPopulation() {
        String data = "generation,individual,ruleCount,survivalTime,distanceTicks,noveltyScore,chaosTimeoutMs,hasChaosRules\n";
        int foundCount = 0;
        
        Logger::log("📥 Download population requested");
        
        File root = SPIFFS.open("/");
        if (!root) {
            server.send(500, "text/plain", "Cannot open SPIFFS root");
            return;
        }
        ...
        while (File f = root.openNextFile()) {
            ...
            if (name.startsWith("/pop_gen_") && name.endsWith(".bin")) {
                Logger::logf("  Processing: %s", name.c_str());
                
                File file = SPIFFS.open(name, FILE_READ);
                if (!file) { f.close(); continue; }
                
                size_t fileSize = file.size();
                if (fileSize < sizeof(FileHeader)) {
                    Logger::logf("  ❌ File too small: %d bytes", fileSize);
                    file.close(); f.close(); continue;
                }
                
                uint32_t magic;
                uint16_t version, popSize;
                uint32_t generation, expId;
                
                if (file.read((uint8_t*)&magic, 4) != 4) { file.close(); f.close(); continue; }
                if (file.read((uint8_t*)&version, 2) != 2) { file.close(); f.close(); continue; }
                if (file.read((uint8_t*)&popSize, 2) != 2) { file.close(); f.close(); continue; }
                if (file.read((uint8_t*)&generation, 4) != 4) { file.close(); f.close(); continue; }
                if (file.read((uint8_t*)&expId, 4) != 4) { file.close(); f.close(); continue; }
                
                if (magic != 0x47454E45) {
                    Logger::logf("  ❌ Invalid magic: 0x%08X", magic);
                    file.close(); f.close(); continue;
                }
                
                Logger::logf("  ✅ Generation %d, %d individuals, version 0x%04X", 
                             generation, popSize, version);
                
                for (int i = 0; i < popSize && i < 16; i++) {
                    uint8_t ruleCount;
                    if (file.read(&ruleCount, 1) != 1) break;
                    if (ruleCount > MAX_RULES) ruleCount = MAX_RULES;
                    
                    file.seek(file.position() + ruleCount * sizeof(BehaviorRule));
                    
                    uint32_t survival, dist;
                    float novelty;
                    if (file.read((uint8_t*)&survival, 4) != 4) break;
                    if (file.read((uint8_t*)&dist, 4) != 4) break;
                    if (file.read((uint8_t*)&novelty, 4) != 4) break;
                    
                    uint16_t chaosTimeoutMs = 0;
                    uint8_t hasChaosRules = 0;
                    
                    if (version >= 0x0005) {
                        file.seek(file.position() + 17);
                        if (file.read((uint8_t*)&chaosTimeoutMs, 2) != 2) break;
                        file.seek(file.position() + 2);
                        if (version >= 0x0006) {
                            if (file.read(&hasChaosRules, 1) != 1) break;
                            file.seek(file.position() + 2);
                        }
                    }
                    ...
                    data += String(generation) + "," + String(i) + "," + String(ruleCount) + ","
                          ...
                          + String(chaosTimeoutMs) + "," + String(hasChaosRules ? "true" : "false") + "\n";
                    foundCount++;
                }
                file.close();
            }
        ...
        Logger::logf("📊 Population download: %d individuals found", foundCount);
        
        if (foundCount == 0) {
            data += "No population data found. Please run evolution first.\n";
        }
...
void setup() {
    ...
    Logger::log("  🛡️ 序列化鲁棒性修复 (2026-09-03)");
    Logger::log("  ├── serializeIndividual() 带边界检查");
    Logger::log("  ├── deserializeIndividual() 带版本兼容");
    Logger::log("  ├── savePopulation() 精确计算totalSize");
    Logger::log("  ├── loadPopulation() 完整性校验");
    Logger::log("  └── handleDownloadPopulation() 结构化解析");
    Logger::log("  🎯 3轮混沌保障: chaosTimeout=800~1800ms");
...
void loop() {
    ...
    static uint32_t lastChaosCheck = 0;
    if (millis() - lastChaosCheck > 100) {
        lastChaosCheck = millis();
        if (MotorController::getMotorState() == STATE_STUCK) {
            uint32_t totalStuck = MotorController::getTotalStuckTime();
            if (totalStuck > MotorController::getChaosTimeoutMs() + 500) {
                Serial.printf("[CHAOS] 🔥 LOOP FORCE TRIGGER! totalStuck=%lu\n", totalStuck);
                MotorController::startChaos();
            }
        }
    }
    ...
    if (Serial.available()) {
        ...
        if (cmd == "ls" || cmd == "list") {
            ...
            Serial.println("========================================");
            Serial.println("可用命令 (v9.21-RobustFixed):");
