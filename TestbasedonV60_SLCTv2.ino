...
private:
    static int leftBase;
    static int rightBase;
    static bool calibrated;
    
public:
    static void calibrate() {
        Logger::log("🔧 开始传感器基准值校准...");
        Logger::log("⚠️ 请确保传感器前方无遮挡！");
        ...
        Logger::logf("✅ 校准完成: 左基准=%d, 右基准=%d", leftBase, rightBase);
    }
...
bool SensorCalibration::calibrated = false;
...
private:
    ...
    static int targetLeftSpeed;
    static int targetRightSpeed;
...
public:
    ...
    static void setSpeed(int left, int right) {
        // 重新校准增益 - 消除右转偏航
        const float leftGain  = 1.15f;
        const float rightGain = 1.05f;
        ...
        float outL = leftSpeed * leftGain;
        float outR = rightSpeed * rightGain;
    ...
    static int getTargetLeftSpeed() { return targetLeftSpeed; }
    static int getTargetRightSpeed() { return targetRightSpeed; }
};

int Motor::leftSpeed = 0;
int Motor::rightSpeed = 0;
int Motor::targetLeftSpeed = 0;
int Motor::targetRightSpeed = 0;
...
public:
    ...
    static int getLeftSpeed() {
        ...
        int speed = (leftEncoderCount - lastLeft) * 1000 / (now - lastTime);
    ...
    static int getRightSpeed() {
        ...
        uint32_t now = millis();
        if (now - lastTime < 100) return 0;
        int speed = (rightEncoderCount - lastRight) * 1000 / (now - lastTime);
        lastRight = rightEncoderCount;
        lastTime = now;
        return speed;
    }
    
    // 【修复】双维度卡死检测 - 使用不同的变量名避免冲突
    static bool isStuck(int& stuckType) {
        int32_t lDelta = leftDelta();
        int32_t rDelta = rightDelta();
        uint32_t totalDeltaVal = totalDelta();  // 【修复】使用不同变量名
        
        // 判定1：绝对卡死
        bool absoluteStuck = (abs(lDelta) < 2 && abs(rDelta) < 2);
        
        // 判定2：原地旋转卡死
        int leftSpd = getLeftSpeed();
        int rightSpd = getRightSpeed();
        ...
        if (absoluteStuck) {
            stuckType = 0;
            return true;
        } else if (spinningStuck) {
            stuckType = 1;
            Logger::logf("🔄 旋转卡死: 转速差=%d, 总脉冲=%lu", speedDiff, totalDeltaVal);
            return true;
        }
        return false;
    }
...
public:
    ...
    static int getObstacleLevel() {
        return SensorCalibration::getObstacleLevel();
    }
    
    static bool hasObstacle() {
        return SensorCalibration::hasObstacle();
    }
    
    static bool isClear() {
        return SensorCalibration::isClear();
    }
    
    static bool isDanger() {
        return SensorCalibration::isDanger();
    }
};
...
public:
    ...
    void init() {
        ...
        Logger::log("✅ Population initialized (v7.1)");
    }
...
public:
    ...
    void init() {
        ...
        Logger::logf("✅ Evolution initialized (v7.1), generation %lu", currentGen);
    }
    ...
    void start() {
        ...
        currentIdx = 0;
        state = STATE_LOAD;
        Logger::log("▶ Evolution started (v7.1 - Escape Priority)");
    }
...
private:
    ...
    void runTest() {
        ...
        if (!inChaos) {
            ...
            switch (obstacleLevel) {
                case 3: baseSpeed = 40; break;
                case 2: baseSpeed = 60; break;
            ...
            if (abs(diff) < SENSOR_DIFF_DEAD_ZONE) {
                ...
                pwmLeft = baseSpeed + wander;
                pwmRight = baseSpeed - wander;
            } else if (obstacleLevel >= 2) {
                float ratio = constrain(abs(diff) / 2500.0, 0.0, 1.0);
                int turnAmount = baseSpeed * ratio * 0.35;
            ...
            pwmLeft = constrain(pwmLeft, 20, 200);
            pwmRight = constrain(pwmRight, 20, 200);
    ...
    DilemmaType detectDilemma() {
        ...
        if (Encoders::isStuck(stuckTypeLocal)) {
            if (stuckStart == 0) {
                stuckStart = now;
                stuckType = stuckTypeLocal;
            }
            if (now - stuckStart > STUCK_THRESHOLD_MS) {
                Logger::logf("⛔ 卡死检测: %s", stuckTypeLocal == 0 ? "绝对卡死" : "旋转卡死");
        ...
        if (isDanger) {
            ...
            if (collisionCounter > COLLISION_THRESHOLD) {
                Logger::logf("💥 持续碰撞: %d次", collisionCounter);
                return DILEMMA_COLLISION;
            }
        } else if (collisionCounter > 0) {
            collisionCounter = max(0, (int)collisionCounter - 1);
        }
        ...
        if (leftDir != 0 && rightDir != 0) {
            if (leftDir != lastLeftDir || rightDir != lastRightDir) {
                swingCounter++;
                uint32_t totalDeltaVal = Encoders::totalDelta();
                if (swingCounter > SWING_THRESHOLD && totalDeltaVal < 15) {
                    Logger::logf("🔄 反复摇摆: %d次", swingCounter);
                    return DILEMMA_SWING;
                }
            }
        } else {
            if (swingCounter > 0) swingCounter -= 1;
        ...
        if (hasObs) {
            ...
            if (now - lastObstacleChange > EXPLORE_STALL_MS) {
                Logger::logf("⏸ 探索停滞: %lu ms", now - lastObstacleChange);
    ...
    void executeChaosStrategy(uint8_t strategy, int intensity) {
        ...
        switch(strategy) {
            case 0:
                Motor::setSpeed(140 * scale, 140 * scale);
                break;
            case 1: {
                int rotSpeed = 130 * scale;
                int dir = (millis() / 500) % 2 ? 1 : -1;
                Motor::setSpeed(dir * rotSpeed, -dir * rotSpeed);
                break;
            }
            case 2: {
                int randL = random(-120 * scale, 121 * scale);
                int randR = random(-120 * scale, 121 * scale);
                Motor::setSpeed(randL, randR);
                break;
            }
            case 3: {
                int turnSpeed = 140 * scale;
                int dir = (millis() / 300) % 2 ? 1 : -1;
                Motor::setSpeed(dir * turnSpeed, -dir * turnSpeed * 0.8);
                break;
            }
            case 4: {
                int backSpeed = -130 * scale;
    ...
    void evaluate() {
        ...
        int survivalCount = 0;
        for (int i = 0; i < POPULATION_SIZE; i++) {
            if (pop.genes[i].getFitness() >= 0.4f) {
                survivalCount++;
            }
        }
        ...
        Logger::logf("📊 Survival count: %d (fitness >= 0.4)", survivalCount);
...
public:
    ...
    void init() {
        ...
        server.on("/status", HTTP_GET, [this]() {
            ...
            s += "sensor_L_raw:" + String(Sensors::leftRaw()) + "\n";
            s += "sensor_R_raw:" + String(Sensors::rightRaw()) + "\n";
...
<head>
    ...
    <title>Car Evolution v7.1</title>
    <style>
...
<body>
<div class='box'>
    <h1>🚗 Evolution v7.1</h1>
    <div class='version'>全面修复版 | 双维度卡死检测 | 电机增益校准</div>
    <div class='subtitle'>
    ...
    <div class='sensor-box'>
        <div style='color:#888;font-size:11px;margin-bottom:4px;'>传感器ADC值（校准后）</div>
        <div class='sensor-row'>
...
<div class='footer'>
    <span>✅ v7.1 全面修复</span> | 双维度卡死检测 | 电机增益校准
</div>
...
void setup() {
    Logger::init();
    Logger::log("========================================");
    Logger::log("  Car Evolution v7.1 - 全面修复版");
    Logger::log("  脱困能力优先 | 不能脱困→淘汰");
    Logger::log("========================================");
    Logger::log("  ✅ 修复电机增益不平衡（右转问题）");
    Logger::log("  ✅ 双维度卡死检测（绝对卡死+旋转卡死）");
    Logger::log("  ✅ 传感器基准值自动校准");
    Logger::log("  ✅ 速度平滑过渡（防突变）");
    Logger::log("  ✅ 转向死区（小偏差直行）");
    Logger::log("  ✅ 修复碰撞计数器衰减");
    Logger::log("========================================");
